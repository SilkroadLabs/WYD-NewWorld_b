...
//	//FILE* fpTxt = fopen("MeshTextureList.txt", "w");
//	//if (!fpTxt)
//	//{
//	//	LOG_WRITELOG("Cannot open MeshTextureList.txt for writing\n");
//	//	return 0;
//	//}
//
//	//for (int i = 0; i < 2048; ++i)
//	//{
//	//	fprintf(fpTxt, "%s,%c,%u,%u\n",
//	//		m_stModelTextureList[i].szFileName,
//	//		m_stModelTextureList[i].cAlpha,
//	//		m_stModelTextureList[i].dwLastUsedTime,
//	//		m_stModelTextureList[i].dwShowTime);
//	//}
//
//	//fclose(fpTxt);
//
...
{
	char szTxt[128];
	sprintf(szTxt, "%s", MeshTextureList_Path);

	FILE* fpTxt = fopen(szTxt, "r");
	if (!fpTxt)
	{
		LOG_WRITELOG("Cannot open MeshTextureList_with_ID.txt\n");
		return 0;
	}

	char line[512];
	int id = 0;
	while (fgets(line, sizeof(line), fpTxt))
	{
		// Formato: ID: szFileName,cAlpha,dwLastUsedTime,dwShowTime
		sscanf(line, "%d: %254[^,],%c,%u,%u",
			&id,
			m_stModelTextureList[id].szFileName,
			&m_stModelTextureList[id].cAlpha,
			&m_stModelTextureList[id].dwLastUsedTime,
			&m_stModelTextureList[id].dwShowTime);
	}
