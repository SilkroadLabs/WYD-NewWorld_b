...
int TMSelectCharScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	//	g_pObjectManager->m_stMobData.HomeTownX = pCharLogin->PosX;
	//	g_pObjectManager->m_stMobData.HomeTownY = pCharLogin->PosY;
	//	(g_pObjectManager->m_cShortSkill, g_pObjectManager->m_stMobData.ShortSkill, 4);   // <-- Arrumar skills da barra bugados
	//	memcpy(&g_pObjectManager->m_cShortSkill[4], pCharLogin->ShortSkill, sizeof(pCharLogin->ShortSkill));
...
void TMSelectCharScene::ReloadCharList(RELOAD_CHARLIST_TYPE type)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		/*if (pSelChar->Equip[i][14].sIndex >= 3980 && pSelChar->Equip[i][14].sIndex <= 3994)
		{
			m_pHuman[i]->m_cMount = 1;
			m_pHuman[i]->m_cLastMount = 1;
		

			int valor = 0;

			if (pSelChar->Equip[i][14].sIndex == 3980 || pSelChar->Equip[i][14].sIndex == 3983 || pSelChar->Equip[i][14].sIndex == 3986)
			{
				valor = 342;
			}

			if (pSelChar->Equip[i][14].sIndex == 3982 || pSelChar->Equip[i][14].sIndex == 3985 || pSelChar->Equip[i][14].sIndex == 3988)
			{
				valor = 344;
			}
			...
			m_pHuman[i]->m_nMountSkinMeshType = BASE_DefineSkinMeshType(nClass);
			m_pHuman[i]->m_stMountLook.Mesh0 = g_pItemList[sIndex2].nIndexMesh;
			m_pHuman[i]->m_stMountLook.Mesh1 = m_pHuman[i]->m_stMountLook.Mesh0;
			m_pHuman[i]->m_stMountLook.Skin0 = g_pItemList[sIndex2].nIndexTexture;
			m_pHuman[i]->m_stMountLook.Skin1 = m_pHuman[i]->m_stMountLook.Skin0;
			m_pHuman[i]->m_sMountIndex = sIndex2;

		}*/

		if (pSelChar->Equip[i][14].sIndex >= 2360 && pSelChar->Equip[i][14].sIndex <= 2389)
		{
			m_pHuman[i]->m_cMount = 1;
			m_pHuman[i]->m_cLastMount = 1;

			int sIndex2 = pSelChar->Equip[i][14].sIndex - 2045;
			...
			if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 2)
				valor = 0;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 10)
				valor = 1;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 30)
				valor = 2;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 40)
				valor = 3;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 60)
				valor = 4;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 70)
				valor = 5;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 80)
				valor = 6;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 90)
				valor = 7;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 110)
				valor = 8;

			else if (pSelChar->Equip[i][14].stEffect[1].cEffect <= 120)
				valor = 9;
			...
			m_pHuman[i]->m_stMountSanc.Sanc0 = valor;
			m_pHuman[i]->m_stMountSanc.Sanc1 = valor;
