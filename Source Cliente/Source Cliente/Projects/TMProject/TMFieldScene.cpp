...
int TMFieldScene::m_bAutotrade = 0;
int TMFieldScene::m_bCurrentSlotLeilao = 0;
int TMFieldScene::m_bCurrentTipeLeilao = -1;
int TMFieldScene::m_nCurrentSlobankLeilao;
int TMFieldScene::m_nLeilaoStoreCurrentPage = 0; // Página atual
int TMFieldScene::m_nLeilaoStoreTotalPages = 0;  // Total de páginas

int TMFieldScene::OnKeyVisibleLeilao(char iCharCode, int lParam)
{
	if (iCharCode != 'b' && iCharCode != 'B')
		return 0;

	if (m_pLeilaoStore)
	{
		OnControlEvent(3000063u, 0);
	}

	return 1;
}

void TMFieldScene::LeilaoStoreNextPage() {

	if (m_nLeilaoStoreCurrentPage < m_nLeilaoStoreTotalPages)
	{
		m_nLeilaoStoreCurrentPage++;
	}

	if (m_pInvenPanel->m_bVisible)
	{
		SendLeilaoRequest(m_bCurrentTipeLeilao, 0);
	}
	else
	{
		SendLeilaoRequest(m_bCurrentTipeLeilao, 1);
	}
}

void TMFieldScene::LeilaoStorePreviousPage() {

	m_nLeilaoStoreCurrentPage--;
	if (m_nLeilaoStoreCurrentPage <= -1)
		m_nLeilaoStoreCurrentPage = 0;
	if (m_pInvenPanel->m_bVisible)
	{
		SendLeilaoRequest(m_bCurrentTipeLeilao, 0);
	}
...
int TMFieldScene::SendLeilaoOp(int itemType, int alvo)
{

	char szMessage[128]{};
	sprintf(szMessage, "Bem vindo a loja %s", m_stAutoTrade.List[m_bCurrentSlotLeilao].LojinhaName);
 

	m_pMessagePanel->SetMessage(szMessage, 5000);
	m_pMessagePanel->SetVisible(1, 1);
	 
	MSG_STANDARDPARM2 stParm2{};
	stParm2.Header.Type = 0xAF2;
	stParm2.Header.ID = m_pMyHuman->m_dwID;
	stParm2.Parm1 = alvo;
	stParm2.Parm2 = itemType;
...
int TMFieldScene::SendLeilaoRequest(int itemType, int action)
{

	MSG_GetLeilaoStores req{};
	req.Header.ID = g_pObjectManager->m_dwCharID;
	req.Header.Type = 0xAF1;
	req.itemType = itemType;
	req.currentpage = m_nLeilaoStoreCurrentPage;
	req.totalpage = m_nLeilaoStoreTotalPages;
	SendOneMessage((char*)&req, sizeof(req));
	UpdateElementesLeilaoStore();
	return 1;
}

void TMFieldScene::UpdateElementesLeilaoStore() {

	if (m_pLeilaoStore && m_pLeilaoStore->m_bVisible == 0)
		return;
...
#pragma region Labels
	...
	auto currentpage = m_nLeilaoStoreCurrentPage;
	...
	if (LabelCurrentPage)
	{
		snprintf(strSexy, sizeof(strSexy), "%d/%d", currentpage, m_nLeilaoStoreTotalPages);
		LabelCurrentPage->SetText((char*)strSexy, 0);
	}

	auto LabelTituloPanel = static_cast<SText*>(m_pControlContainer->FindControl(3810069));
	...
	if (LabelTituloPanel)
	{
		...
		{
		case 0:
			LabelTituloPanel->SetText((char*)"Itens em negociação", 0);
			break;
		case 1:
			LabelTituloPanel->SetText((char*)"Negociados por Donate", 0);
			break;
		case 2:
			LabelTituloPanel->SetText((char*)"Negociados por Gold", 0);
			break;
		case 3:
			LabelTituloPanel->SetText((char*)"Negociados por Barras(1b)", 0);
			break;
		case -1:
			LabelTituloPanel->SetText((char*)"Lojinha - Leilão Store", 0);
			break;
		}
	}
...
#pragma region BTNS
		...
		auto oBtnAbrirLoja = (SButton*)m_pControlContainer->FindControl(3002211);

		if (oBtnAbrirLoja)
		{
			if (m_bAutotrade)
			{
				oBtnAbrirLoja->m_GCPanel.nTextureSetIndex = 761;
				oBtnAbrirLoja->m_pAltText->SetText((char*)"Fechar lojinha", 0); 
			}
			else
			{
				oBtnAbrirLoja->m_GCPanel.nTextureSetIndex = 760;
				oBtnAbrirLoja->m_pAltText->SetText((char*)"Criar lojinha", 0);
			}
		}
		 
#pragma endregion

#pragma region Slots Grid Status and Label price
		...
		for (int i = 0; i < MAX_SLOT_LEILAO_AUTOTRADE; ++i)
		{
			...
			if (m_stAutoTrade.List[i].OwnerId <= 0)
				continue;
			  
			if (m_stAutoTrade.List[i].Item.sIndex <= 0)
				continue;
			...
			auto bb2 =  m_pGridLeilaoStore->GetItem(x2, y2);
			if (!bb2)
				continue;

			if (m_pMyHuman->m_dwID == m_stAutoTrade.List[i].OwnerId)
			{
				if (m_stAutoTrade.List[i].Item.sIndex == m_stAutoTrade.List[i].Item.sIndex)
				{
					enableItem = 0xFFFF0000;
				}
				else
				{
					enableItem = 0xFFFFFFFF;
				}
			}
			else
			{
			...
			bb2->m_GCObj.dwColor = enableItem;


			if (g_pObjectManager->m_stItemCargo[m_stAutoTrade.List[i].SlotBank].sIndex)
			{
				if (m_pMyHuman->m_dwID != m_stAutoTrade.List[i].OwnerId &&
					g_pObjectManager->m_stItemCargo[m_stAutoTrade.List[i].SlotBank].sIndex != m_stAutoTrade.List[i].Item.sIndex)
				{
					enableItem = 0xFFFFFFFF;
				}
				if (m_pMyHuman->m_dwID == m_stAutoTrade.List[i].OwnerId &&
				...
				int Page = m_stAutoTrade.List[i].SlotBank / 40; // Determina a página do item
				if (Page < 4) // Verifica se a página está dentro do intervalo válido
				{
					int mod40 = m_stAutoTrade.List[i].SlotBank % 40;
					int x = mod40 % 5;
					int y = mod40 / 5;

					auto bb = this->m_pCargoGridList[Page]->GetItem(x, y);
					if (!bb)
						continue;

					bb->m_GCObj.dwColor = enableItem;
				}
			}

			
			char szText[128] = "";
			if (m_stAutoTrade.List[i].Categoria == 1)
			{
				sprintf(szText, "%d Donate", m_stAutoTrade.List[i].Price);
			}
			else if (m_stAutoTrade.List[i].Categoria == 2)
			{
				sprintf(szText, "%d Gold", m_stAutoTrade.List[i].Price);
			}
			else if (m_stAutoTrade.List[i].Categoria == 3)
			{
				sprintf(szText, "%d Bis", m_stAutoTrade.List[i].Price);
			}			 
		}
#pragma endregion
...
void TMFieldScene::SendReqLeilaoBuy()//aqui compra o item
{
	...
	sendPacket.itemType = static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].Categoria);
	sendPacket.itemTax = static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].TaxRate);
	sendPacket.SlotBank = static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].SlotBank);
	memcpy(&sendPacket.item, &m_stAutoTrade.List[m_bCurrentSlotLeilao].Item, sizeof(STRUCT_ITEM));

	std::cout << "\n  *******************  ";
	std::cout << "\n Compra enviada :: m_bCurrentSlotLeilao " << m_bCurrentSlotLeilao;
	std::cout << "  OwnerId " << m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId;
	std::cout << "  Price " << static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].Price);
	std::cout << "  Categoria " << static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].Categoria);
	std::cout << "  TaxRate " << static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].TaxRate) ;
	...
	printf("Pacote de início de negociação preparado.\n");
...
{ 
	...
	auto pAutoTrade = reinterpret_cast<MSG_LeilaoStore*>(pStd);

	// Limpa o grid somente uma vez antes de adicionar todos os itens
	pGridLeilaoStore->Empty();
	...
	for (int i = 0; i < MAX_SLOT_LEILAO_AUTOTRADE; ++i) {
		...
		// Copia as informações do item recebido para a estrutura local
		...
		// Configura o grid para exibir os itens
		...
		// Adiciona o item ao grid na posição correta
		if (pAutoTrade->List[i].Item.sIndex > 0) {
			...
			pGridLeilaoStore->AddItem(new SGridControlItem(nullptr, pItemRecovery, 0.0f, 0.0f), nx, ny);
		}
	} 
...
void TMFieldScene::AltLeilaoAction(int action)
{
	for (int nCargoIndex = 0; nCargoIndex < 160; ++nCargoIndex)
	{
		// Verifica se o item no índice atual é válido
		if (g_pObjectManager->m_stItemCargo[nCargoIndex].sIndex)
		{
			int Page = nCargoIndex / 40; // Determina a página do item
			if (Page < 4) // Verifica se a página está dentro do intervalo válido
			{
				// Calcula as coordenadas x e y na página
				int mod40 = nCargoIndex % 40;
				int x = mod40 % 5;
				int y = mod40 / 5;

				// Obtém o item da grade
				auto bb = m_pCargoGridList[Page]->GetItem(x, y);

				// Verifica se o item é válido
				if (!bb)
					continue; // Pular itens que não são válidos

				// Atualiza a cor do item para vermelho se o item já está no leilão
...
void TMFieldScene::SetVisibleLeilaoAction(int bShow, int bCargo)
{
	...
	if (m_pLeilaoStore)
	{
		...
		m_pLeilaoStore->SetVisible(bShow);

		//std::cout << "\n entrada m_pGridInvList[0]->m_eGridType " << (int)m_pGridInvList[0]->m_eGridType;


		if (bShow == 1)
		{
			m_dwDelayBuscaLeilao = g_pApp->m_pTimerManager->GetServerTime();
			...
			m_pCargoGridList[3]->m_eGridType = TMEGRIDTYPE::GRID_TRADEINV2; 
		...
		else
		{
			...
			m_pCargoGridList[3]->m_eGridType = TMEGRIDTYPE::GRID_CARGO;
		...
		//std::cout << "\n saida m_pGridInvList[0]->m_eGridType " << (int)m_pGridInvList[0]->m_eGridType;
	}
}
 
int TMFieldScene::OnPacketUpdateDonate(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_UpdateDonate*>(pStd);

	g_pClientInfo->Info.Cash = p->Cash;
	if (g_pClientInfo->Info.Cash < 0)
		g_pClientInfo->Info.Cash = 0;

	strncpy(g_pClientInfo->Info.Pix, p->Pix, sizeof(g_pClientInfo->Info.Pix));

	auto DonateInv = (SText*)m_pControlContainer->FindControl(250099);
	auto PixTx4 = (SText*)m_pControlContainer->FindControl(188517);

	DonateInv->SetText(strFmt("Diamantes: %d", g_pClientInfo->Info.Cash), 0);
	DonateInv->SetTextColor(TNColor::CornBlue);

	PixTx4->SetText(strFmt("Diamantes: %d", g_pClientInfo->Info.Cash), 0);
	PixTx4->SetTextColor(TNColor::CornBlue);

	auto PixTx3 = (SText*)m_pControlContainer->FindControl(188516);
	PixTx3->SetText(strFmt("Sua Chave: %s", g_pClientInfo->Info.Pix), 0);
...
stReqBuy.TargetID = m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId;
stReqBuy.Pos = m_bCurrentSlotLeilao;
stReqBuy.Price = m_stAutoTrade.List[m_bCurrentSlotLeilao].Price;
stReqBuy.Tax = m_stAutoTrade.List[m_bCurrentSlotLeilao].TaxRate;
stReqBuy.item = m_stAutoTrade.List[m_bCurrentSlotLeilao].Item;
std::cout << "\n m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId " << m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId;
SendOneMessage((char*)&stReqBuy, sizeof(stReqBuy));*/
//
//void TMFieldScene::SendReqLeilaoBuy()
//{ 
//	pStartNegotiation sendPacket = {};
//
//	sendPacket.Header.ID = g_pObjectManager->m_dwCharID;
//	sendPacket.Header.Type = MSG_ReqBuy_Opcode; // Tipo do pacote de negociação
//	sendPacket.targetPlayerId = m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId;
//	sendPacket.itemSlot = m_bCurrentSlotLeilao;
//	sendPacket.itemPrice = static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].Price);
//	sendPacket.itemType = static_cast<int>(m_stAutoTrade.List[m_bCurrentSlotLeilao].Categoria);
//	sendPacket.itemTax = m_stAutoTrade.List[m_bCurrentSlotLeilao].TaxRate;
...
void OpenStore(int Warp)
{
	...
	Packet.Type = MSG_REQShopList_Opcode;
...
int TMFieldScene::InitializeScene()
{
	...
	m_pCash = (SText*)m_pControlContainer->FindControl(250099);
	if (m_pCash)
		m_pCash->SetText("0", 0);

	m_pOldCashStore = (SPanel*)m_pControlContainer->FindControl(265686);
	if (m_pOldCashStore)
		m_pOldCashStore->SetVisible(0);
	...
	m_pChatListnotice->m_nPosX = 370.0f * RenderDevice::m_fHeightRatio;
	...
	m_pGridNewSlot1 = (SGridControl*)m_pControlContainer->FindControl(1655571);
	m_pGridNewSlot2 = (SGridControl*)m_pControlContainer->FindControl(1655581);

	//CRIA OS GRIDS MUTIPLOS	
	for (int i = 0; i < 5; i++) {
		m_pJephiGrid[i] = (SGridControl*)m_pControlContainer->FindControl(117901 + i);
		m_pJephiGrid[i]->m_bDrawGrid = 1;
		m_pJephiGrid[i]->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
	}
	for (int i = 0; i < 15; i++) {
		m_pTrajeGrid[i] = (SGridControl*)m_pControlContainer->FindControl(1882001 + i);
		m_pTrajeGrid[i]->m_bDrawGrid = 1;
		m_pTrajeGrid[i]->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
	}
	...
	auto unkpanel1 = (SPanel*)m_pControlContainer->FindControl(71989);

	if (unkpanel1)
		unkpanel1->m_bVisible = 0;


	auto unkpanel2 = (SPanel*)m_pControlContainer->FindControl(910450);

	if (unkpanel2)
		unkpanel2->m_bVisible = 0;


	auto unkpanel3 = (SPanel*)m_pControlContainer->FindControl(456654);

	if (unkpanel3)
		unkpanel3->m_bVisible = 0;

	auto unkpanel4 = (SPanel*)m_pControlContainer->FindControl(1616);

	if (unkpanel4)
		unkpanel4->m_bVisible = 0;
	

	auto unkpanel5 = (SPanel*)m_pControlContainer->FindControl(71921);

	if (unkpanel5)
		unkpanel5->m_bVisible = 0;
	 

	auto unkpanel8 = (SPanel*)m_pControlContainer->FindControl(81921);
	...
	if (m_pLeilaoStore)
	m_pLeilaoStore->m_bVisible = 0;

	m_pGridLeilaoStore = (SGridControl*)m_pControlContainer->FindControl(3200011);

	if (m_pGridLeilaoStore)
	{
		m_pGridLeilaoStore->m_eGridType = TMEGRIDTYPE::GRID_LEILAOSTORE;
		m_pGridLeilaoStore->Empty();
	}
	 
	m_pAutoTradeValuePanel = (SPanel*)m_pControlContainer->FindControl(565885);
	if (m_pAutoTradeValuePanel)
	{

		m_pAutoTradeValuePanel->SetPos(((float)g_pDevice->m_dwScreenWidth * 0.5f) - (m_pAutoTradeValuePanel->m_nWidth * 0.8f),
			((float)g_pDevice->m_dwScreenHeight * 0.5f) - (m_pAutoTradeValuePanel->m_nHeight * 0.5f));
		m_pAutoTradeValuePanel->SetVisible(0);
	}
	...
	g_pObjectManager->m_pCamera->m_fMaxCamLen = 20.0f;
	if (m_pPartyPanel)
...
#pragma region Novas Janelas
	//enter world
	auto reCaptcha = (SPanel*)m_pControlContainer->FindControl(1319001);
	auto RankingPainel = (SPanel*)m_pControlContainer->FindControl(1919400);
	auto SkinsMontaria = (SPanel*)m_pControlContainer->FindControl(1882000);
	auto MailButton = (SPanel*)m_pControlContainer->FindControl(1212300);
	auto StoreConfirm = (SPanel*)m_pControlContainer->FindControl(121200);
	auto PainelPix = (SPanel*)m_pControlContainer->FindControl(188500);
	auto PainelNovato = (SPanel*)m_pControlContainer->FindControl(117900);
	auto Droplist = (SPanel*)m_pControlContainer->FindControl(179500);
	auto StoreConfirm2 = (SPanel*)m_pControlContainer->FindControl(192500);
	auto DonateStore = (SPanel*)m_pControlContainer->FindControl(199300);
	auto DPainel = (SPanel*)m_pControlContainer->FindControl(981300);
	auto NewPainel = (SPanel*)m_pControlContainer->FindControl(98300);	

	DPainel->SetVisible(0);
	DonateStore->SetVisible(0);
	reCaptcha->SetVisible(0);
	RankingPainel->SetVisible(0);
	SkinsMontaria->SetVisible(0);
	MailButton->SetVisible(0);
	StoreConfirm->SetVisible(0);
	PainelPix->SetVisible(0);
	PainelNovato->SetVisible(0);
	Droplist->SetVisible(0);
	StoreConfirm2->SetVisible(0);
	NewPainel->SetVisible(0);
	gLoopSendInfo = false;
#pragma endregion
...
int TMFieldScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == B_IG_OK)// 65886
	{
		...
		if (strcmp(inputText, "all"))
		{
			...
			case 1:
			{
				...
				char szText[16]{};// valor do gold troca
			...
			break;
			case 3: //Cria o mob para a loja do leilão
			{
				...
				for (int i = 0; i < MAX_SLOT_LEILAO_AUTOTRADE; i++)
				{
					m_stAutoTrade.List[i].LojinhaName[23] = '\0';
					m_stAutoTrade.List[i].LojinhaName[22] = '\0';
					sprintf_s(m_stAutoTrade.List[i].LojinhaName, "%s", pInputText->GetText());
				}
				m_stAutoTrade.List->OwnerId = m_pMyHuman->m_dwID;
	...
	if (idwControlID == B_INV_CLOSE)
	{
		...
		/*auto DonateWindow = (SPanel*)m_pControlContainer->FindControl(999300);
		auto PainelNew = (SPanel*)m_pControlContainer->FindControl(98300);
		auto ShopPainel = (SPanel*)m_pControlContainer->FindControl(65695);
		ShopPainel->SetTextureSetIndex(-261);
		PainelNew->SetVisible(0);
		DonateWindow->SetVisible(0);*/
...
/*BOTOES DO SISTEMA DE LEILÃO*/
	if (idwControlID == 3000060u) // categoria donate
	{
		m_pGridLeilaoStore->Empty();
		m_pGridLeilaoStore->m_eGridType = TMEGRIDTYPE::GRID_LEILAOSTORE;
		m_bCurrentTipeLeilao = 1;
		m_nLeilaoStoreCurrentPage = 1;
		AltLeilaoAction(0);
	...
	{ 
		...
		if (BuscaleilaoStore)
		{
			m_pControlContainer->SetFocusedControl(BuscaleilaoStore);
		}
		return 1;
	}
	if (idwControlID == 65794)
	{
	...
	if (idwControlID == 3002211) // 65794u
	{
		if (!m_pMyHuman->IsInTown())
			return 1;
		...
		if (m_bAutotrade)
		{
			...
			sprintf_s(stWhisper.MobName, "fecharloja");
	...
	if (idwControlID >= 565811 && idwControlID <= 565813)
	{
		m_bCurrentTipeLeilao = (idwControlID - 565811) + 1;

		auto Generico1 = static_cast<SButton*>(m_pControlContainer->FindControl(565811));
		auto Generico2 = static_cast<SButton*>(m_pControlContainer->FindControl(565812));
		auto Generico3 = static_cast<SButton*>(m_pControlContainer->FindControl(565813));
		...
		case 1:
		{
			if (Generico1)
			{
				Generico1->m_GCPanel.nTextureSetIndex = 246;
			}
			if (Generico2)
			{
				Generico2->m_GCPanel.nTextureSetIndex = 280;
			}
			if (Generico3)
			{
				Generico3->m_GCPanel.nTextureSetIndex = 280;
			}
			message = "Item agora disponível para compra com donate (cash)!";
			break;
		}
		case 2:
		{
			if (Generico1)
			{
				Generico1->m_GCPanel.nTextureSetIndex = 280;
			}
			if (Generico2)
			{
				Generico2->m_GCPanel.nTextureSetIndex = 246;
			}
			if (Generico3)
			{
				Generico3->m_GCPanel.nTextureSetIndex = 280;
			}
			message = "Item agora disponível para compra com ouro!";
			break;
		}
		case 3:
		{
			...
			message = "Item agora disponível para compra com barras de ouro!";
			break;
		}
		default:
			break;
		...
		if (!message.empty())
		{
			m_pMessagePanel->SetMessage(message.c_str(), 10000);
			m_pMessagePanel->SetVisible(1, 1);
		}
	...
	if (idwControlID == 3010060)//idwControlID do Botão ID
	{
		LeilaoStoreNextPage();
		return 1;
	}
	if (idwControlID == 3010061)//idwControlID do Botão ID
	{
		std::cout << "\n m_bCurrentTipeLeilao " << m_bCurrentTipeLeilao;
		LeilaoStorePreviousPage();
		return 1;
	}
	if (idwControlID == 565887)
	{
		m_pAutoTradeValuePanel->m_bVisible = 0;
		return 1;
	}
	if (idwControlID == 565886)
	{
		auto pInputText = static_cast<SEditableText*>(m_pControlContainer->FindControl(565889));

		char* inputText = pInputText->GetText();

		int inputTextLen = strlen(inputText);
		std::cout << "\n m_bCurrentTipeLeilao " << m_bCurrentTipeLeilao;
		if (m_bCurrentTipeLeilao == -1)
		{

			m_pMessagePanel->SetMessage((char*)"Selecione qual tipo de venda para o item", 5000);
			m_pMessagePanel->SetVisible(1, 1);
			return 1;
		}
		...
		if (strcmp(inputText, "all"))
		{
			...
			if (m_nCurrentSlobankLeilao >= 0 && m_bCurrentSlotLeilao >= 0)
			{

				auto pGridLeilaoStore = m_pGridLeilaoStore;

				if (!pGridLeilaoStore) {
					std::cout << "Grid de Leilão não encontrado!" << std::endl;
					return 1;
				}

				SGridControl* pParent = pGridLeilaoStore;
				SGridControlItem* pAutoTradeItem = pParent->GetAtItem(0, 0);

				SGridControl* pCargoGrid = m_pCargoGridList[m_nCurrentSlobankLeilao / 40];
				SGridControlItem* pCargoItem = pCargoGrid->GetAtItem(m_nCurrentSlobankLeilao % 40 % 5, m_nCurrentSlobankLeilao % 40 / 5);

				std::cout << "\n  m_nCurrentSlobankLeilao " << m_nCurrentSlobankLeilao;
				std::cout << "\n  m_bCurrentTipeLeilao " << m_bCurrentTipeLeilao;

				pAddItemAutoTrade SendPacket = {};
				if (!pCargoItem) {
					std::cout << "Item no Cargo não encontrado!" << std::endl;
					return 1;
				}
				...
				SendPacket.Header.ID = g_pObjectManager->m_dwCharID;
				SendPacket.Header.Type = 0x397C;

				SendPacket.SlotBank = m_nCurrentSlobankLeilao;
				SendPacket.itemPrice = static_cast<int>(nInputValue);
				SendPacket.itemType = static_cast<int>(m_bCurrentTipeLeilao);

				memcpy(&SendPacket.Name, m_stAutoTrade.List[m_bCurrentSlotLeilao].LojinhaName, sizeof(m_stAutoTrade.List[m_bCurrentSlotLeilao].LojinhaName));

				memcpy(&SendPacket.Item, pItem, sizeof(STRUCT_ITEM));

				pParent->m_nTradeMoney = static_cast<int>(nInputValue);
				SendOneMessage((char*)&SendPacket, sizeof(pAddItemAutoTrade));
				m_nCurrentSlobankLeilao = -1;
				m_stAutoTrade.List[m_bCurrentSlotLeilao].Categoria = -1;
			}
...
// Novas Janelas - button control
	auto painelteleport = (SPanel*)m_pControlContainer->FindControl(98300);
	auto GamePainel = (SPanel*)m_pControlContainer->FindControl(981300);
	auto StorePainel = (SPanel*)m_pControlContainer->FindControl(199300);
	auto BuyConfirm = (SPanel*)m_pControlContainer->FindControl(192500);
	auto DropList = (SPanel*)m_pControlContainer->FindControl(179500);
	auto PainelPix = (SPanel*)m_pControlContainer->FindControl(188500);
	auto PainelTrajes = (SPanel*)m_pControlContainer->FindControl(1882000);
	auto PainelRanking = (SPanel*)m_pControlContainer->FindControl(1919400);
	auto PainelCaptcha = (SPanel*)m_pControlContainer->FindControl(1319001);

	auto PainelJephi = (SPanel*)m_pControlContainer->FindControl(117900);
	auto MailPainel = (SPanel*)m_pControlContainer->FindControl(121200);
	auto MailButton = (SPanel*)m_pControlContainer->FindControl(1212300);

	auto player = g_pObjectManager->m_stMobData;

	int SendHandleID = idwControlID;
	...
	case TRAJEBUTTON:
	{
		...
		auto PainelTrajes = (SPanel*)m_pControlContainer->FindControl(1882000);
		for (int i = 0; i < 15; i++) {
			m_pTrajeGrid[i]->Empty();
		}
		PainelTrajes->SetVisible(0);
	}break;
	case CAPTCHABUTTON:
	{
		int Alias = CaptchaButton + 100;
		ReqAlias(Alias);
		PainelCaptcha->SetVisible(0);
	}break;
	case OPENRANKING:
	{
		GamePainel->SetVisible(0);
		PainelRanking->SetVisible(1);
		ReqAlias(4);
	}break;
	case 1919430: //Ranking - Anterior
	{
		...
		if (g_pClientInfo->Ranking.State == 0) {
			if (g_pClientInfo->cConfigR.RankingPage >= 2) {
				auto Pos1 = (SText*)m_pControlContainer->FindControl(1919410);
				auto Pos2 = (SText*)m_pControlContainer->FindControl(1919411);
				auto Pos3 = (SText*)m_pControlContainer->FindControl(1919412);
				auto Pos4 = (SText*)m_pControlContainer->FindControl(1919413);
				auto Pos5 = (SText*)m_pControlContainer->FindControl(1919414);
				auto Pos6 = (SText*)m_pControlContainer->FindControl(1919415);
				auto Pos7 = (SText*)m_pControlContainer->FindControl(1919416);
				auto Pos8 = (SText*)m_pControlContainer->FindControl(1919417);
				auto Pos9 = (SText*)m_pControlContainer->FindControl(1919418);
				auto Pos10 = (SText*)m_pControlContainer->FindControl(1919419);
				auto Page = (SText*)m_pControlContainer->FindControl(1919420);

				g_pClientInfo->cConfigR.RankingPage = g_pClientInfo->cConfigR.RankingPage - 1;

				int Pos = g_pClientInfo->cConfigR.RankingPage * 10;
				Pos -= 10;

				Page->SetText(strFmt("%d", g_pClientInfo->cConfigR.RankingPage), 0);
				Pos1->SetText(strFmt("%d. [%s][%d][%s][%s]", 1 + Pos, g_pClientInfo->Ranking.RankName[0 + Pos], g_pClientInfo->Ranking.RankLevel[0 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[0 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[0 + Pos]]), 0);
				Pos2->SetText(strFmt("%d. [%s][%d][%s][%s]", 2 + Pos, g_pClientInfo->Ranking.RankName[1 + Pos], g_pClientInfo->Ranking.RankLevel[1 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[1 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[1 + Pos]]), 0);
				Pos3->SetText(strFmt("%d. [%s][%d][%s][%s]", 3 + Pos, g_pClientInfo->Ranking.RankName[2 + Pos], g_pClientInfo->Ranking.RankLevel[2 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[2 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[2 + Pos]]), 0);
				Pos4->SetText(strFmt("%d. [%s][%d][%s][%s]", 4 + Pos, g_pClientInfo->Ranking.RankName[3 + Pos], g_pClientInfo->Ranking.RankLevel[3 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[3 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[3 + Pos]]), 0);
				Pos5->SetText(strFmt("%d. [%s][%d][%s][%s]", 5 + Pos, g_pClientInfo->Ranking.RankName[4 + Pos], g_pClientInfo->Ranking.RankLevel[4 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[4 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[4 + Pos]]), 0);
				Pos6->SetText(strFmt("%d. [%s][%d][%s][%s]", 6 + Pos, g_pClientInfo->Ranking.RankName[5 + Pos], g_pClientInfo->Ranking.RankLevel[5 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[5 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[5 + Pos]]), 0);
				Pos7->SetText(strFmt("%d. [%s][%d][%s][%s]", 7 + Pos, g_pClientInfo->Ranking.RankName[6 + Pos], g_pClientInfo->Ranking.RankLevel[6 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[6 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[6 + Pos]]), 0);
				Pos8->SetText(strFmt("%d. [%s][%d][%s][%s]", 8 + Pos, g_pClientInfo->Ranking.RankName[7 + Pos], g_pClientInfo->Ranking.RankLevel[7 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[7 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[7 + Pos]]), 0);
				Pos9->SetText(strFmt("%d. [%s][%d][%s][%s]", 9 + Pos, g_pClientInfo->Ranking.RankName[8 + Pos], g_pClientInfo->Ranking.RankLevel[8 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[8 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[8 + Pos]]), 0);
				Pos10->SetText(strFmt("%d. [%s][%d][%s][%s]", 10 + Pos, g_pClientInfo->Ranking.RankName[9 + Pos], g_pClientInfo->Ranking.RankLevel[9 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[9 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[9 + Pos]]), 0);
			}
		}
		if (g_pClientInfo->Ranking.State == 1) {
			if (g_pClientInfo->cConfigR.RankingPage >= 2) {
				auto Pos1 = (SText*)m_pControlContainer->FindControl(1919410);
				auto Pos2 = (SText*)m_pControlContainer->FindControl(1919411);
				auto Pos3 = (SText*)m_pControlContainer->FindControl(1919412);
				...
				Page->SetText(strFmt("%d", g_pClientInfo->cConfigR.RankingPage), 0);
				Pos1->SetText(strFmt("%d. [%s][%d][%s][%s]", 1 + Pos, g_pClientInfo->Ranking.RankName[0 + Pos], g_pClientInfo->Ranking.PvP[0 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[0 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[0 + Pos]]), 0);
				Pos2->SetText(strFmt("%d. [%s][%d][%s][%s]", 2 + Pos, g_pClientInfo->Ranking.RankName[1 + Pos], g_pClientInfo->Ranking.PvP[1 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[1 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[1 + Pos]]), 0);
				Pos3->SetText(strFmt("%d. [%s][%d][%s][%s]", 3 + Pos, g_pClientInfo->Ranking.RankName[2 + Pos], g_pClientInfo->Ranking.PvP[2 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[2 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[2 + Pos]]), 0);
				Pos4->SetText(strFmt("%d. [%s][%d][%s][%s]", 4 + Pos, g_pClientInfo->Ranking.RankName[3 + Pos], g_pClientInfo->Ranking.PvP[3 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[3 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[3 + Pos]]), 0);
				Pos5->SetText(strFmt("%d. [%s][%d][%s][%s]", 5 + Pos, g_pClientInfo->Ranking.RankName[4 + Pos], g_pClientInfo->Ranking.PvP[4 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[4 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[4 + Pos]]), 0);
				Pos6->SetText(strFmt("%d. [%s][%d][%s][%s]", 6 + Pos, g_pClientInfo->Ranking.RankName[5 + Pos], g_pClientInfo->Ranking.PvP[5 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[5 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[5 + Pos]]), 0);
				Pos7->SetText(strFmt("%d. [%s][%d][%s][%s]", 7 + Pos, g_pClientInfo->Ranking.RankName[6 + Pos], g_pClientInfo->Ranking.PvP[6 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[6 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[6 + Pos]]), 0);
				Pos8->SetText(strFmt("%d. [%s][%d][%s][%s]", 8 + Pos, g_pClientInfo->Ranking.RankName[7 + Pos], g_pClientInfo->Ranking.PvP[7 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[7 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[7 + Pos]]), 0);
				Pos9->SetText(strFmt("%d. [%s][%d][%s][%s]", 9 + Pos, g_pClientInfo->Ranking.RankName[8 + Pos], g_pClientInfo->Ranking.PvP[8 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[8 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[8 + Pos]]), 0);
				Pos10->SetText(strFmt("%d. [%s][%d][%s][%s]", 10 + Pos, g_pClientInfo->Ranking.RankName[9 + Pos], g_pClientInfo->Ranking.PvP[9 + Pos], Evolution[g_pClientInfo->Ranking.RankEvolution[9 + Pos] - 1], Classe[g_pClientInfo->Ranking.RankClasse[9 + Pos]]), 0);
			}
		}
	}break;
	...
	case 1212301:
	{
		ReqAlias(2);
		MailButton->SetVisible(0);
		MailPainel->SetVisible(1);
	}break;
	case 121201:
	{
		ReqAlias(3);
		MailButton->SetVisible(0);
		MailPainel->SetVisible(0);
	}break;
	case JEPHIBUTTON:
	{
		if (g_pClientInfo->cConfigR.WindowControl == JEPHI) {
			if (JephiButton == 0 || JephiButton == 1) {
				g_pClientInfo->cConfigR.Jephi = JephiButton;

				int Gold[2] = { 250000, 100000 };
				int Index[2] = { 413, 412 };
				int sIndex[2] = { 420, 419 };

				BuyConfirm->SetVisible(1);
				PainelJephi->SetVisible(0);
				int Price = 10;

				g_pClientInfo->cConfigR.ShopItemQnt = 1;

				//GERA ITEM GRID
				auto item = new STRUCT_ITEM;
				memset(item, 0, sizeof(STRUCT_ITEM));
				item->sIndex = Index[g_pClientInfo->cConfigR.Jephi];
				
				auto BuyConfirmGrid = (SGridControl*)m_pControlContainer->FindControl(192520);
				BuyConfirmGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
				BuyConfirmGrid->Empty();
				auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
				BuyConfirmGrid->AddItem(pItem, 0, 0);
				////////

				int Qnt = 0;

				for (int i = 0; i < MAX_CARRY; i++) {
					if (player.Carry[i].sIndex == sIndex[g_pClientInfo->cConfigR.Jephi])
						Qnt += player.Carry[i].stEffect[0].cValue;
				}

				auto ConfirmTx1 = (SText*)m_pControlContainer->FindControl(192530);
				ConfirmTx1->SetText(strFmt("%d", g_pClientInfo->cConfigR.ShopItemQnt), 0);
				ConfirmTx1->SetTextColor(TNColor::CornBlue);

				auto ConfirmTx2 = (SText*)m_pControlContainer->FindControl(192531);
				ConfirmTx2->SetText(strFmt("Restos: %d", (Price * g_pClientInfo->cConfigR.ShopItemQnt)), 0);
				ConfirmTx2->SetTextColor(TNColor::CornBlue);

				auto ConfirmTx3 = (SText*)m_pControlContainer->FindControl(192532);
				ConfirmTx3->SetText(strFmt("Gold: %d", (g_pClientInfo->cConfigR.ShopItemQnt * Gold[g_pClientInfo->cConfigR.Jephi])), 0);
				ConfirmTx3->SetTextColor(TNColor::CornBlue);

				auto ConfirmTx4 = (SText*)m_pControlContainer->FindControl(192533);
				ConfirmTx4->SetText(strFmt("Restos Atual: %d", Qnt), 0);
		...
		if (g_pClientInfo->cConfigR.WindowControl == ITEMLEVEL) {
			SendItemLevel(g_pClientInfo->cConfigR.ItemLevel[JephiButton], g_pClientInfo->cConfigR.ShopItemQnt);

			for (int i = 0; i < 5; i++) {
				m_pJephiGrid[i]->Empty();
			}
	...
	case OPEMPIX:
	{
		g_pClientInfo->cConfigR.PixValue = 0;

		ReqAlias(1);

		GamePainel->SetVisible(0);
		PainelPix->SetVisible(1);
		auto PixTx1 = (SText*)m_pControlContainer->FindControl(188513);
		PixTx1->SetText(strFmt("%d", g_pClientInfo->cConfigR.PixValue), 0 );
		PixTx1->SetTextColor(TNColor::CornBlue);

		auto PixTx2 = (SText*)m_pControlContainer->FindControl(188514);
		PixTx2->SetText(strFmt("Sem Taxa"), 0);
	...
	case 188510:
	{
		g_pClientInfo->cConfigR.PixValue += 100;
	...
	case 188511:
	{
		g_pClientInfo->cConfigR.PixValue += 1000;
	...
	case SENDPIX:
	{
		auto Pix = (SText*)m_pControlContainer->FindControl(188501);
		SendPix(g_pClientInfo->cConfigR.PixValue, Pix->GetText());
	}break;
	case OPENDROPLIST:
	{
		auto GamePainel = (SPanel*)m_pControlContainer->FindControl(981300);
		...
		DropList->SetVisible(1);
		g_pClientInfo->cConfigR.DropListPage = 0;
		g_pClientInfo->cConfigR.DropListRegion = 0;

		auto NewWindowTX1 = (SText*)m_pControlContainer->FindControl(179511);
		auto NewWindowTX2 = (SText*)m_pControlContainer->FindControl(179512);
		auto NewWindowTX3 = (SText*)m_pControlContainer->FindControl(179513);
		auto NewWindowTX4 = (SText*)m_pControlContainer->FindControl(179514);
		auto NewWindowTX5 = (SText*)m_pControlContainer->FindControl(179515);
		auto NewWindowTX6 = (SText*)m_pControlContainer->FindControl(179516);
		auto NewWindowTX7 = (SText*)m_pControlContainer->FindControl(179517);
		auto NewWindowTX8 = (SText*)m_pControlContainer->FindControl(179518);
		auto NewWindowTX9 = (SText*)m_pControlContainer->FindControl(179519);
		auto NewWindowTX10 = (SText*)m_pControlContainer->FindControl(179520);

		NewWindowTX1->SetText(strFmt("Armia"), 0);
		NewWindowTX2->SetText(strFmt("Erion"), 0);
		NewWindowTX3->SetText(strFmt("Arzan"), 0);
		NewWindowTX4->SetText(strFmt("Deserto"), 0);
		NewWindowTX5->SetText(strFmt("Gelo"), 0);
		NewWindowTX6->SetText(strFmt("Dungeon"), 0);
		NewWindowTX7->SetText(strFmt("Submundo"), 0);
		NewWindowTX8->SetText(strFmt("Kefra"), 0);
		NewWindowTX9->SetText(strFmt("Lan N"), 0);
		NewWindowTX10->SetText(strFmt("Lan M"), 0);

		auto PageTx = (SText*)m_pControlContainer->FindControl(179521);
		PageTx->SetText(strFmt("%d/2", (g_pClientInfo->cConfigR.DropListPage + 1)), 0);
	}break;
	case DROPLISTBUTTON:
	{
		if (g_pClientInfo->cConfigR.DropListRegion != 0) {
			int Page = g_pClientInfo->cConfigR.DropListPage * 10;
			int State = DropListButton + Page;
			OpenDroplist(g_pClientInfo->DropList.MobName[State], g_pClientInfo->cConfigR.DropListRegion);
		}
		if (g_pClientInfo->cConfigR.DropListRegion == 0) {
			int Page = g_pClientInfo->cConfigR.DropListPage * 10;
			g_pClientInfo->cConfigR.DropListRegion = DropListButton + 1 + Page;

			g_pClientInfo->cConfigR.DropListPage = 0;
			RequestDropList(g_pClientInfo->cConfigR.DropListRegion);
		}
	}break;
	case 179531:
	{
		g_pClientInfo->cConfigR.DropListPage++;

		if (g_pClientInfo->cConfigR.DropListRegion == 0) {

			if (g_pClientInfo->cConfigR.DropListPage > 1)
				g_pClientInfo->cConfigR.DropListPage = 1;
			...
			NewWindowTX1->SetText(strFmt("Lan A"), 0 );
			NewWindowTX2->SetText(strFmt("Vale"), 0);
			NewWindowTX3->SetText(strFmt(""), 0);
			NewWindowTX4->SetText(strFmt(""), 0);
			NewWindowTX5->SetText(strFmt(""), 0);
			NewWindowTX6->SetText(strFmt(""), 0);
			NewWindowTX7->SetText(strFmt(""), 0);
			NewWindowTX8->SetText(strFmt(""), 0);
			NewWindowTX9->SetText(strFmt(""), 0);
			NewWindowTX10->SetText(strFmt(""), 0);

			auto PageTx = (SText*)m_pControlContainer->FindControl(179521);
			PageTx->SetText(strFmt("%d/2", (g_pClientInfo->cConfigR.DropListPage + 1)), 0);
		}
		if (g_pClientInfo->cConfigR.DropListRegion != 0) {
			if (g_pClientInfo->cConfigR.DropListPage >= g_pClientInfo->cConfigR.DropListTpages - 1)
				g_pClientInfo->cConfigR.DropListPage = g_pClientInfo->cConfigR.DropListTpages - 1;
			...
			NewWindowTX1->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 0]), 0);
			NewWindowTX2->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 1]), 0);
			NewWindowTX3->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 2]), 0);
			NewWindowTX4->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 3]), 0);
			NewWindowTX5->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 4]), 0);
			NewWindowTX6->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 5]), 0);
			NewWindowTX7->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 6]), 0);
			NewWindowTX8->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 7]), 0);
			NewWindowTX9->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 8]), 0);
			NewWindowTX10->SetText(strFmt("%s", g_pClientInfo->DropList.MobName[page + 9]), 0);

			auto PageTx = (SText*)m_pControlContainer->FindControl(179521);
			PageTx->SetText(strFmt("%d/%d", (g_pClientInfo->cConfigR.DropListPage + 1), g_pClientInfo->cConfigR.DropListTpages), 0);
		}
	}break;
	case 179530:
	{
		...
		if (g_pClientInfo->cConfigR.DropListRegion == 0) {
			...
			if (g_pClientInfo->cConfigR.DropListPage < 0)
				g_pClientInfo->cConfigR.DropListPage = 0;
	...
	case CLOSECHAT:
	{
		auto CHATBTS = (SPanel*)m_pControlContainer->FindControl(65672);
		auto CHATOPCOES = (SPanel*)m_pControlContainer->FindControl(90128);
		auto CHATBTCOMUM = (SPanel*)m_pControlContainer->FindControl(90113);
		auto CHATGUIA = (SPanel*)m_pControlContainer->FindControl(65667);
		auto CHATFUNDO = (SPanel*)m_pControlContainer->FindControl(65943);
		auto CHATDIGITAR = (SPanel*)m_pControlContainer->FindControl(65670);

		gChatView = !gChatView;

		if (gChatView == TRUE)
		{
			CHATBTS->SetVisible(0);
			CHATGUIA->SetVisible(0);
			CHATFUNDO->SetVisible(0);
			CHATGUIA->SetVisible(0);
			CHATDIGITAR->SetVisible(0);
			CHATBTCOMUM->SetVisible(0);
		}
		if (gChatView == FALSE)
		{
			CHATBTS->SetVisible(1);
			CHATGUIA->SetVisible(1);
			CHATFUNDO->SetVisible(1);
			CHATGUIA->SetVisible(1);
		}
	}break;
	case OPENPVPRANKING:
	...
	case DONATEBUTTON:
	{
		g_pClientInfo->cConfigR.WindowControl = DONATESTORE;
		g_pClientInfo->cConfigR.ShopItemPos = DonateButton;

		g_pClientInfo->cConfigR.ShopItemQnt = 1;

		int ItemIndex = g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][0] * g_pClientInfo->cConfigR.ShopItemQnt;

		if (ItemIndex <= 0 || ItemIndex >= MAX_ITEMLIST)
			break;

		StorePainel->SetVisible(0);
		BuyConfirm->SetVisible(1);

		int Price = g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][1] * g_pClientInfo->cConfigR.ShopItemQnt;

		auto item = new STRUCT_ITEM;
		memset(item, 0, sizeof(STRUCT_ITEM));
		item->sIndex = g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][0];
		...
		auto ConfirmTx2 = (SText*)m_pControlContainer->FindControl(192531);
		ConfirmTx2->SetText(strFmt("Price: %d", Price), 0);
		...
		auto ConfirmTx3 = (SText*)m_pControlContainer->FindControl(192532);
		ConfirmTx3->SetText(strFmt("Saldo Atual: %d", g_pClientInfo->Info.Cash), 0);
		ConfirmTx3->SetTextColor(TNColor::CornBlue);

		auto ConfirmTx4 = (SText*)m_pControlContainer->FindControl(192533);
		ConfirmTx4->SetText(strFmt("Saldo Final: %d", (g_pClientInfo->Info.Cash - Price)), 0);
		ConfirmTx4->SetTextColor(TNColor::CornBlue);
	...
	case 192512:
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			g_pClientInfo->cConfigR.ShopItemQnt += 1;

			if (g_pClientInfo->cConfigR.ShopItemQnt > 120)
				g_pClientInfo->cConfigR.ShopItemQnt = 120;

			int Price = g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][1] * g_pClientInfo->cConfigR.ShopItemQnt;

			if (Price > g_pClientInfo->Info.Cash) {
				g_pClientInfo->cConfigR.ShopItemQnt -= 1;
				Price -= g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][1];
			}
		...
		if (g_pClientInfo->cConfigR.WindowControl == JEPHI) {
			...
			int Price = 10 * g_pClientInfo->cConfigR.ShopItemQnt;
			...
			auto ConfirmTx2 = (SText*)m_pControlContainer->FindControl(192531);
			ConfirmTx2->SetText(strFmt("Restos: %d", Price), 0);
			ConfirmTx2->SetTextColor(TNColor::CornBlue);
	...
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			g_pClientInfo->cConfigR.ShopItemQnt += 10;
			...
			if (Price > g_pClientInfo->Info.Cash) {
				g_pClientInfo->cConfigR.ShopItemQnt -= 10;
				Price -= g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][1] * 10;
			}
	...
	case 192515:
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			...
			if (Price > g_pClientInfo->Info.Cash) {
				g_pClientInfo->cConfigR.ShopItemQnt -= 120;
				Price -= g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][g_pClientInfo->cConfigR.ShopItemPos][1] * 120;
			}
	...
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			if (g_pClientInfo->cConfigR.ShopItemQnt > 1)
				g_pClientInfo->cConfigR.ShopItemQnt -= 1;
	...
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			SendReqDonateShop(g_pClientInfo->cConfigR.ShopStore, g_pClientInfo->cConfigR.ShopPage, g_pClientInfo->cConfigR.ShopItemPos, g_pClientInfo->cConfigR.ShopItemQnt);

			auto BuyConfirmGrid = (SGridControl*)m_pControlContainer->FindControl(192520);
			BuyConfirmGrid->Empty();

			BuyConfirm->SetVisible(0);
			StorePainel->SetVisible(1);
		}
		if (g_pClientInfo->cConfigR.WindowControl == JEPHI) {
			SendJephi(g_pClientInfo->cConfigR.Jephi, g_pClientInfo->cConfigR.ShopItemQnt);

			auto BuyConfirmGrid = (SGridControl*)m_pControlContainer->FindControl(192520);
			BuyConfirmGrid->Empty();

			BuyConfirm->SetVisible(0);
			PainelJephi->SetVisible(1);
		}

	}break;
	case OPENSTORE:
	{
		g_pClientInfo->cConfigR.ShopStore = 0;
		g_pClientInfo->cConfigR.ShopPage = 0;

		auto Grupo = (SPanel*)m_pControlContainer->FindControl(475136);
		auto Compositor = (SPanel*)m_pControlContainer->FindControl(81921);
		auto Compositor2 = (SPanel*)m_pControlContainer->FindControl(6432);
		auto Compositor3 = (SPanel*)m_pControlContainer->FindControl(86017);
		auto xPainel = (SPanel*)m_pControlContainer->FindControl(864);
		auto xPainel2 = (SPanel*)m_pControlContainer->FindControl(1054256);
		auto Loja = (SPanel*)m_pControlContainer->FindControl(65692);
		auto SkillLoja = (SPanel*)m_pControlContainer->FindControl(65602);
		auto SkillInv = (SPanel*)m_pControlContainer->FindControl(65567);
		auto cPainel = (SPanel*)m_pControlContainer->FindControl(65696);
		auto Bau = (SPanel*)m_pControlContainer->FindControl(65686);

		auto bufftempo = (SPanel*)m_pControlContainer->FindControl(5749);
		auto ElementoModoFoto3 = (SPanel*)m_pControlContainer->FindControl(69633);
		bufftempo->SetVisible(0);
		ElementoModoFoto3->SetVisible(0);
		...
		CHATDIGITAR->SetVisible(0);
		CHATBTCOMUM->SetVisible(0);
		g_pClientInfo->cConfigR.gModoChat = TRUE;

		Grupo->SetVisible(0);
		Compositor->SetVisible(0);
		Compositor2->SetVisible(0);
		Compositor3->SetVisible(0);
		xPainel->SetVisible(0);
		xPainel2->SetVisible(0);
		Loja->SetVisible(0);
		SkillLoja->SetVisible(0);
		SkillInv->SetVisible(0);
		cPainel->SetVisible(0);
		Bau->SetVisible(0);
		...
		auto TxShopPage = (SText*)m_pControlContainer->FindControl(199516);
		TxShopPage->SetText(strFmt("%d/%d", (g_pClientInfo->cConfigR.ShopPage + 1), tPages), 0);

		for (int i = 0; i < 15; i++) {
			auto item = new STRUCT_ITEM;
			memset(item, 0, sizeof(STRUCT_ITEM));
			item->sIndex = g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][0];

			auto ShopGrid = (SGridControl*)m_pControlContainer->FindControl(199401 + i);
			ShopGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;			
			ShopGrid->Empty();

			if (item->sIndex != 0) {
				auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
				ShopGrid->AddItem(pItem, 0, 0);
			}

			auto TxShopGrid = (SText*)m_pControlContainer->FindControl(199501 + i);
			TxShopGrid->SetText(strFmt("Price: %d", g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][1]), 0);
			TxShopGrid->SetTextColor(TNColor::CornBlue);

			if (g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][0] == 0)
				TxShopGrid->SetText(strFmt(""), 0);
		}
	}break;
	case 199322:
	{
	...
	case 199323:
	{
		g_pClientInfo->cConfigR.ShopStore = 1;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199324:
	{
		g_pClientInfo->cConfigR.ShopStore = 2;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199320:
	{
		if (g_pClientInfo->cConfigR.ShopPage == 0)
			break;

		g_pClientInfo->cConfigR.ShopPage -= 1;
	...
	case 199321:
	{
		if (g_pClientInfo->cConfigR.ShopPage == 4)
			break;
	...
	case FILTROBTN:
	{
		auto NewWindowTX1 = (SText*)m_pControlContainer->FindControl(98401);
		auto NewWindowTX2 = (SText*)m_pControlContainer->FindControl(98402);
		auto NewWindowTX3 = (SText*)m_pControlContainer->FindControl(98403);
		auto NewWindowTX4 = (SText*)m_pControlContainer->FindControl(98404);
		auto NewWindowTX5 = (SText*)m_pControlContainer->FindControl(98405);
		auto NewWindowTX6 = (SText*)m_pControlContainer->FindControl(98406);
		auto NewWindowTX7 = (SText*)m_pControlContainer->FindControl(98407);
		auto NewWindowTX8 = (SText*)m_pControlContainer->FindControl(98408);
		auto NewWindowTX9 = (SText*)m_pControlContainer->FindControl(98409);
		auto NewWindowTX10 = (SText*)m_pControlContainer->FindControl(98410);
		...
		NewWindowTX7->SetText(strFmt(""), 0);
		NewWindowTX8->SetText(strFmt(""), 0);
		NewWindowTX9->SetText(strFmt("Limpar"), 0);
		NewWindowTX10->SetText(strFmt("Ativar/Desativar"), 0);

		g_pClientInfo->cConfigR.WindowControl = 24;
		painelteleport->SetVisible(1);
		GamePainel->SetVisible(0);
	}break;

	case OPT1:
	{
		if (g_pClientInfo->cConfigR.WindowControl == 24) {
			OpenStore(100);
		}
	}break;
	case OPT2:
...
{ 
	...
	if (OnKeyVisibleLeilao(iCharCode, lParam))
		return 1;
...
int TMFieldScene::OnKeyDownEvent(unsigned int iKeyCode)
{
	...
	if (m_pLeilaoStore && m_pLeilaoStore->m_bVisible)
		return 1;
...
#define VK_U 85
	...
	auto pPix = (SEditableText*)m_pControlContainer->FindControl(188501);

	if (m_pEditChat->IsFocused() == false && pEdit->IsFocused() == false && pPix->IsFocused() == false)
	{
		if (iKeyCode == VK_F)
		{
			auto painelteleport = (SPanel*)m_pControlContainer->FindControl(98300);
			...
			NewWindowTX7->SetText("", 0);
			NewWindowTX8->SetText("", 0);
			NewWindowTX9->SetText("Limpar", 0);
			NewWindowTX10->SetText("Ativar/Desativar", 0);
		...
		if (iKeyCode == VK_U)
		{
			...
			auto CHATFUNDO = (SPanel*)m_pControlContainer->FindControl(65943);
			auto CHATDIGITAR = (SPanel*)m_pControlContainer->FindControl(65670);
			auto personagem = (SPanel*)m_pControlContainer->FindControl(65696);
			auto CloseChat = (SPanel*)m_pControlContainer->FindControl(190200);
			auto GamePainel = (SPanel*)m_pControlContainer->FindControl(981300);

			auto ElementoModoFoto1 = (SPanel*)m_pControlContainer->FindControl(65628);
			auto ElementoModoFoto3 = (SPanel*)m_pControlContainer->FindControl(69633);
			auto ElementoModoFoto6 = (SPanel*)m_pControlContainer->FindControl(65796);
			auto ElementoModoFoto7 = (SPanel*)m_pControlContainer->FindControl(65795);
			auto ElementoModoFoto8 = (SPanel*)m_pControlContainer->FindControl(65794);
			auto ElementoModoFoto9 = (SPanel*)m_pControlContainer->FindControl(65793);
			auto ElementoModoFoto10 = (SPanel*)m_pControlContainer->FindControl(65790);
			auto ElementoModoFoto11 = (SPanel*)m_pControlContainer->FindControl(65791);
			auto bufftempo = (SPanel*)m_pControlContainer->FindControl(5749);

			auto PainelInv = (SPanel*)m_pControlContainer->FindControl(589832);
			auto painelteleport = (SPanel*)m_pControlContainer->FindControl(98300);

			g_pClientInfo->cConfigR.gModoFoto = !g_pClientInfo->cConfigR.gModoFoto;
			SysMsgChat(" ");
			SysMsgChat(" ");
			SysMsgChat(" ");
			SysMsgChat(" ");
			...
			if (g_pClientInfo->cConfigR.gModoFoto == TRUE)
			{
				CHATBTS->SetVisible(0);
				CHATGUIA->SetVisible(0);
				CHATFUNDO->SetVisible(0);
				...
				CHATDIGITAR->SetVisible(0);
				CHATBTCOMUM->SetVisible(0);
				personagem->SetVisible(0);
				ElementoModoFoto1->SetVisible(0);
				ElementoModoFoto3->SetVisible(0);
				ElementoModoFoto6->SetVisible(0);
				ElementoModoFoto7->SetVisible(0);
				ElementoModoFoto8->SetVisible(0);
				ElementoModoFoto9->SetVisible(0);
				ElementoModoFoto10->SetVisible(0);
				ElementoModoFoto11->SetVisible(0);
				bufftempo->SetVisible(0);
				GamePainel->SetVisible(0);
				CloseChat->SetVisible(0);
			}
			if (g_pClientInfo->cConfigR.gModoFoto == FALSE)
			{
				CHATBTS->SetVisible(1);
				CHATGUIA->SetVisible(1);
				CHATFUNDO->SetVisible(1);
				CHATGUIA->SetVisible(1);
				ElementoModoFoto1->SetVisible(1);
				ElementoModoFoto3->SetVisible(1);
				ElementoModoFoto6->SetVisible(1);
				ElementoModoFoto7->SetVisible(1);
				ElementoModoFoto8->SetVisible(1);
				ElementoModoFoto9->SetVisible(1);
				ElementoModoFoto10->SetVisible(1);
				ElementoModoFoto11->SetVisible(1);
				bufftempo->SetVisible(1);
				CloseChat->SetVisible(1);
			}
		}
		if (iKeyCode == VK_D)
		{
			ReqAlias(2);
			auto GamePainel = (SPanel*)m_pControlContainer->FindControl(981300);
			bool visible = GamePainel->IsVisible();

			if (visible == true)
				GamePainel->SetVisible(0);

			if (visible == false)
				GamePainel->SetVisible(1);
		}
	}
...
int TMFieldScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	//o que acontece quando recebe o pacote
	switch (pStd->Type)
	{

	case 0x397B:
		return OnReceivingLeilaoStores(pStd);

	case  pLeilaoStatus:
	{
		auto Pack = (MSG_STANDARDPARM*)pStd;
		m_bAutotrade = Pack->Parm;
		return 1;
	}
	case  0xBF1:
	{
		...
		auto Pack = (st_SaldoLeilao*)pStd;

		char strSexy[128]{}; 
		snprintf(strSexy, sizeof strSexy, "%10d", Pack->Gold);
		auto InfoGoldLeilaoStore = (SText*)m_pControlContainer->FindControl(3300069);
		if (InfoGoldLeilaoStore)
		{
			InfoGoldLeilaoStore->SetText(strSexy, 0);
		}

		snprintf(strSexy, sizeof strSexy, "%d", Pack->Donate);
		auto InfoCashLeilaoStore = (SText*)m_pControlContainer->FindControl(3000069);
		if (InfoCashLeilaoStore)
		{
			InfoCashLeilaoStore->SetText(strSexy, 0);
			InfoCashLeilaoStore->SetText(strSexy, 0);
		}

		auto InfoBisLeilaoStore = (SText*)m_pControlContainer->FindControl(3400069);
		if (InfoBisLeilaoStore)
		{
			SGridControl* pGridInv = nullptr;
			SGridControlItem* pItem = nullptr;
			int totalAmount = Pack->Bis;

			// Define o texto do controle com a quantidade total
			if (totalAmount > 0)
			{
				snprintf(strSexy, sizeof strSexy, "%d", totalAmount);
				InfoBisLeilaoStore->SetText((char*)strSexy, 0);
			}
			else
			{
				InfoBisLeilaoStore->SetText((char*)"0", 0);
			}
		}
	...
	case MSG_ExecCommand:
		return OnPacketExecuteConsole(reinterpret_cast<MSG_STANDARD*>(pStd));
		break;
	case 0x18D:
	...
	case MSG_ShopList_Opcode: //ENTRADA DE PACOTES
		return OnPacketShopList(pStd);
	...
	case MSG_ChatColor:
		return OnPacketChatColor(pStd);
	case MSG_ChatColor2:
		return OnPacketChatColor2(pStd);
	case _MSG_UpdateKeys:
		return OnPacketUpdateKeys(pStd);
	case _MSG_UpdateJoias:
		return OnPacketUpdateJoias(pStd);
	case _MSG_UpdateItemLevel:
		return OnPacketUpdateItemLevel(pStd);
	case _MSG_SendTraje:
		return OnPacketSendTraje(pStd);
	case _MSG_UpdateDonateStore:
		return OnPacketUpdateDonateStore(pStd);
	case _MSG_Recaptcha:
		return OnPacketUpdateRecaptcha(pStd);
	case _MSG_SendRanking:
		return OnPacketSendRanking(pStd);
	case _MSG_MailItem:
		return OnPacketMailItem(pStd);
	case _MSG_ServerInfos:
		return OnPacketServerInfos(pStd);
	case _MSG_SendDListNames:
		return OnPacketSendDListNames(pStd);
	case _MSG_UpdateDropList:
		return OnPacketUpdateDropList(pStd);
	case _MSG_UpdateDonate:
		return OnPacketUpdateDonate(pStd);
		break;
	}
...
int TMFieldScene::FrameMove(unsigned int dwServerTime)
{
	...
	for (int i = 0; i < 32; ++i)
	{
		...
		{
			char szVal[128]{};
			unsigned int sTime = 8 * m_pMyHuman->m_stAffect[i].Time - (dwServerTime - m_dwStartAffectTime[i]) / 1000;
			if (sTime <= 0)
			{
				if (m_pAffectL[i])
				{
	...
	if (m_pRankTimeText && m_bRankTimeOn) //Greentime
	{
		if (m_pGround->m_vecOffsetIndex.x == 1 && m_pGround->m_vecOffsetIndex.y == 31
			...
			|| m_pGround->m_vecOffsetIndex.x == 16 && m_pGround->m_vecOffsetIndex.y == 16 //Armia
			|| m_pGround->m_vecOffsetIndex.x == 19 && m_pGround->m_vecOffsetIndex.y == 15 //Erion
			|| m_pGround->m_vecOffsetIndex.x == 19 && m_pGround->m_vecOffsetIndex.y == 13 //Azran
			|| m_pGround->m_vecOffsetIndex.x == 28 && m_pGround->m_vecOffsetIndex.y == 24 //Gelo
			|| m_pGround->m_vecOffsetIndex.x == 8 && m_pGround->m_vecOffsetIndex.y == 13 //Noatum
				...
				if (!m_bTeleportMsg)
				{
					...
					else
					{
						...
						if ((int)m_pMyHuman->m_vecPosition.x >= 2362 && (int)m_pMyHuman->m_vecPosition.x <= 2370 &&
							(int)m_pMyHuman->m_vecPosition.y >= 3921 && (int)m_pMyHuman->m_vecPosition.y <= 3925)
						{
							m_pMessageBox->SetMessage("Deseja se teleportar para o Hall?", 42949, 0);
							m_pMessageBox->SetVisible(1);
						}
						if ((int)m_pMyHuman->m_vecPosition.x >= 2362 && (int)m_pMyHuman->m_vecPosition.x <= 2370 &&
							(int)m_pMyHuman->m_vecPosition.y >= 3888 && (int)m_pMyHuman->m_vecPosition.y <= 3895)
						{
							m_pMessageBox->SetMessage("Deseja se teleportar para o Kefra?", 67295, 0);
							m_pMessageBox->SetVisible(1);
						}
						if (!m_pMessageBox->IsVisible())
						{
...
int TMFieldScene::SkillUse(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime, int bMoving, TMHuman* pTarget)
{
	...
	//	return 0;   //DEIXAR O BM BUFFAR A SKILL USANDO TRAJE
	...
	if ((unsigned char)cSkillIndex >= 248)
		return GetItemFromGround(dwServerTime);
	if (m_stAutoTrade.List->OwnerId == m_pMyHuman->m_dwID)
		return GetItemFromGround(dwServerTime);
...
int TMFieldScene::MouseClick_NPC(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime)
{
	...
	if (pOver->m_TradeDesc[0])
	{
		...
		for (int i = 0; i < MAX_SLOT_LEILAO_AUTOTRADE; ++i)
		{
			if (m_stAutoTrade.List[i].OwnerId)
			{
				if (m_pMyHuman->m_dwID == m_stAutoTrade.List[i].OwnerId)
					AltLeilaoAction(-1);
				else
	...
	//NOVAS JANELAS - NPC CLICK
	

	if (pOver->m_dwID >= 1000 && (pOver->m_stScore.Reserved & 0xF) == 1)
	{
		if (pOver->m_sHeadIndex == 58 && pOver->m_sHelmIndex == 115) {
			g_pClientInfo->cConfigR.WindowControl = JEPHI;
			auto PainelJephi = (SPanel*)m_pControlContainer->FindControl(117900);
			PainelJephi->SetVisible(1);

			auto TxShopGrid = (SText*)m_pControlContainer->FindControl(117930);
			TxShopGrid->SetText(strFmt("Clique no item escolhido para compor"), 0);
			TxShopGrid->SetTextColor(TNColor::CornBlue);
			...
			for (int i = 0; i < 5; i++) {
				...
				auto JephiGrid = (SGridControl*)m_pControlContainer->FindControl(117901 + i);
				JephiGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
				JephiGrid->Empty();
				auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
				JephiGrid->AddItem(pItem, 0, 0);
			}
			return 1;
		}
...
void TMFieldScene::SetVisibleDonateShop(int bShow)
{
	SGridControl::m_sLastMouseOverIndex = -1;
...
void TMFieldScene::SetVisibleDropList(int bShow)
{
	SGridControl::m_sLastMouseOverIndex = -1;
...
void TMFieldScene::SetInVisibleInputCoin()
{
	...
	if (m_nCurrentSlobankLeilao >= 0)
	{
		m_pCargoGridList[m_nCurrentSlobankLeilao / 40]->GetAtItem(m_nCurrentSlobankLeilao % 40 % 5,	m_nCurrentSlobankLeilao % 40 / 5)->m_GCObj.dwColor = 0xFFFFFFFF;
...
void TMFieldScene::UpdateWindows(int Flag)
{
	if (Flag != 0) {
...
void TMFieldScene::UpdateScoreUI(unsigned int unFlag)
{
	...
	if (!(unFlag & 0x10))
	{
		...
		sprintf(szStr, "%d", pMobData->SkillBonus);
		if (m_pSkBonus)
		...
		char szNameTemp[128]{};
		sprintf(szNameTemp, "[%s]", pMobData->MobName);
		if (m_pCIName)
	...
	if (nExp >= 0 && BASE_Get3DTo2DPos(m_pMyHuman->m_vecPosition.x, m_pMyHuman->m_fHeight + 1.0f, m_pMyHuman->m_vecPosition.y, &nTX, &nTY))
	{
		...
		if (!m_bShowExp)
		{
			if (m_pChatListnotice)
				m_pChatListnotice->AddItem(new SListBoxItem(szStr, 0xFFCCAAFF, 200.0f, -64.0f, 0.0f, 0.0f, 0, 0x77777777, 1, 0));
		}
	}
...
void TMFieldScene::OnESC()
{
	...
	auto DPainel = (SPanel*)m_pControlContainer->FindControl(981300);
	auto NewPainel = (SPanel*)m_pControlContainer->FindControl(98300);
	auto PainelRuna = (SPanel*)m_pControlContainer->FindControl(369001);
	auto PainelInv = (SPanel*)m_pControlContainer->FindControl(589832);
	...
	else if (m_pAutoTradeValuePanel && m_pAutoTradeValuePanel->IsVisible() == 1)
	{
		m_pAutoTradeValuePanel->SetVisible(0);
	}
	else if (m_pInvenPanel->IsVisible() == 1 && m_pLeilaoStore && m_pLeilaoStore->m_bVisible == 1)
	{
		SetVisibleLeilaoAction(0, 0);
	}
	else if (m_pCargoPanel->IsVisible() == 1 && m_pLeilaoStore && m_pLeilaoStore->m_bVisible == 1)
	{
		SetVisibleLeilaoAction(0, 0);
	}
	...
	//NOVAS JANELAS - ESC BUTTON

	else if (DPainel->IsVisible() == 1) {
		DPainel->SetVisible(0);
	}
	else if (Droplist->IsVisible() == 1) {
		auto DropListGrid = (SGridControl*)m_pControlContainer->FindControl(179600);
		DropListGrid->Empty();
		Droplist->SetVisible(0);
	}
	else if (SkinsMontaria->IsVisible() == 1) {
		SkinsMontaria->SetVisible(0);
	}
	else if (RankingPainel->IsVisible() == 1) {
		RankingPainel->SetVisible(0);
	}
	else if (StoreConfirm->IsVisible() == 1) {
		StoreConfirm->SetVisible(0);
	}
	else if (PainelPix->IsVisible() == 1) {
		PainelPix->SetVisible(0);
	}
	else if (PainelNovato->IsVisible() == 1) {
		PainelNovato->SetVisible(0);
	}
	else if (NewPainel->IsVisible() == 1) {
		NewPainel->SetVisible(0);
	}
	else if (DonateStore->IsVisible() == 1) {
		DonateStore->SetVisible(0);
		auto bufftempo = (SPanel*)m_pControlContainer->FindControl(5749);
		auto ElementoModoFoto3 = (SPanel*)m_pControlContainer->FindControl(69633);
		bufftempo->SetVisible(1);
	...
	else if (StoreConfirm2->IsVisible() == 1) {
		StoreConfirm2->SetVisible(0);

		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE)
			DonateStore->SetVisible(1);

		if (g_pClientInfo->cConfigR.WindowControl == JEPHI)
			PainelNovato->SetVisible(1);
	}
...
int TMFieldScene::OnMsgBoxEvent(unsigned int idwControlID, unsigned int idwEvent, unsigned int dwServerTime)
{
	...
	case 601:
	{
		...
		if (m_pTradePanel && !m_pTradePanel->IsVisible())
		{
			...
			char szMyName[128]{};
			char szOPName[128]{};
			sprintf_s(szMyName, "[%s]", m_pMyHuman->m_szName);
			sprintf_s(szOPName, "[%s]", pNode->m_szName);
	...
	case 67295:
	{
		MSG_ReqTp stTeleport{};
		stTeleport.ID = m_pMyHuman->m_dwID;
		stTeleport.Type = _MSG_ReqTp;
		SendOneMessage((char*)&stTeleport, sizeof(stTeleport));
	}
	break;
	//Teleport Hall
	case 42949:
	...
	case 4:
	{
		...
		stApplyBonus.TargetID = m_pGridShop->m_dwMerchantID;
		SendOneMessage((char*)&stApplyBonus, sizeof(stApplyBonus));
	}
	break;
	case 6193:
	...
	case 3200011:
	{
		SendLeilaoOp(0, m_stAutoTrade.List[m_bCurrentSlotLeilao].OwnerId);
		break;
	}
	case 3200012:
	{
...
int TMFieldScene::OnKeyFeedMount(char iCharCode, int lParam)
{
	...
	sprintf_s(stWhisper.MobName, "asdf");
...
int TMFieldScene::OnKeyVisibleInven(char iCharCode, int lParam)
{
	...
	if (m_pLeilaoStore && m_pLeilaoStore->IsVisible())
		return 1;
...
int TMFieldScene::OnPacketCreateMob(MSG_STANDARD* pStd)
{
	...
	std::cout << "\n pCreateMobTrade->Desc " << pCreateMobTrade->Desc;
	printf(" pStd->Type 0x%X", pStd->Type);
...
{
	if (pStd->Parm1 == m_stAutoTrade.List->OwnerId)
	{
		auto pGridLeilaoStore = m_pGridLeilaoStore;
		auto pItem = pGridLeilaoStore->PickupAtItem(pStd->Parm2 % 8, pStd->Parm2 / 8);
...
int TMFieldScene::OnPacketSendTraje(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendTraje*>(pStd);

	auto PainelTrajes = (SPanel*)m_pControlContainer->FindControl(1882000);
	PainelTrajes->SetVisible(1);
	...
	for (int i = 0; i < 15; i++) {	
		auto ITEM = new STRUCT_ITEM;
		memset(ITEM, 0, sizeof(STRUCT_ITEM));
		ITEM->sIndex = g_pClientInfo->Info.Keys[1 + i] + TrajeIndex;		

		if (ITEM->sIndex != 0 && (ITEM->sIndex >= 4500 && ITEM->sIndex <= 4549)) {
			auto pItem = new SGridControlItem(0, ITEM, 0.0f, 0.0f);
			m_pTrajeGrid[i]->AddItem(pItem, 0, 0);
		}
	}

	return 1;
}
int TMFieldScene::OnPacketUpdateDonateStore(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_UpdateDonateStore*>(pStd);

	for (int i = 0; i < 3; i++) {
		for (int x = 0; x < 5; x++) {
			for (int z = 0; z < 15; z++) {
				for (int c = 0; c < 3; c++) {
					g_pClientInfo->Info.cStoreItens[i][x][z][c] = p->cStoreItens[i][x][z][c];
				}
			}
		}
	}
...
int TMFieldScene::OnPacketUpdateRecaptcha(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_Recaptcha*>(pStd);
	...
	auto PainelCaptcha = (SPanel*)m_pControlContainer->FindControl(1319001);
	PainelCaptcha->SetVisible(1);

	auto Desc = (SText*)m_pControlContainer->FindControl(1319002);
	...
	Desc2->SetText(strFmt("[ATENÇÃO] A próxima tentativa será somente em 2 minutos"), 0);
...
int TMFieldScene::OnPacketSendRanking(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendRanking*>(pStd);

	for (int i = 0; i < 50; i++) {
		strncpy(g_pClientInfo->Ranking.RankName[i], p->RankName[i], sizeof(g_pClientInfo->Ranking.RankName[i]));
		g_pClientInfo->Ranking.RankLevel[i] = p->RankLevel[i];
		g_pClientInfo->Ranking.RankClasse[i] = p->RankClasse[i];
		g_pClientInfo->Ranking.RankEvolution[i] = p->RankEvolution[i];
		g_pClientInfo->Ranking.PvP[i] = p->PvP[i];
	}
	g_pClientInfo->Ranking.State = p->State;
	...
	if (p->State == 0) {
		auto Pos = (SText*)m_pControlContainer->FindControl(1919401);
		Pos->SetText(strFmt("Pos"), 0);
		auto Name = (SText*)m_pControlContainer->FindControl(1919402);
		Name->SetText(strFmt("Nick"), 0);
		...
		g_pClientInfo->cConfigR.RankingPage = 1;

		Page->SetText(strFmt("%d", g_pClientInfo->cConfigR.RankingPage), 0);
		Pos1->SetText(strFmt("1. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[0], g_pClientInfo->Ranking.RankLevel[0], Evolution[g_pClientInfo->Ranking.RankEvolution[0] - 1], Classe[g_pClientInfo->Ranking.RankClasse[0]]), 0);
		Pos2->SetText(strFmt("2. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[1], g_pClientInfo->Ranking.RankLevel[1], Evolution[g_pClientInfo->Ranking.RankEvolution[1] - 1], Classe[g_pClientInfo->Ranking.RankClasse[1]]), 0);
		Pos3->SetText(strFmt("3. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[2], g_pClientInfo->Ranking.RankLevel[2], Evolution[g_pClientInfo->Ranking.RankEvolution[2] - 1], Classe[g_pClientInfo->Ranking.RankClasse[2]]), 0);
		Pos4->SetText(strFmt("4. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[3], g_pClientInfo->Ranking.RankLevel[3], Evolution[g_pClientInfo->Ranking.RankEvolution[3] - 1], Classe[g_pClientInfo->Ranking.RankClasse[3]]), 0);
		Pos5->SetText(strFmt("5. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[4], g_pClientInfo->Ranking.RankLevel[4], Evolution[g_pClientInfo->Ranking.RankEvolution[4] - 1], Classe[g_pClientInfo->Ranking.RankClasse[4]]), 0);
		Pos6->SetText(strFmt("6. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[5], g_pClientInfo->Ranking.RankLevel[5], Evolution[g_pClientInfo->Ranking.RankEvolution[5] - 1], Classe[g_pClientInfo->Ranking.RankClasse[5]]), 0);
		Pos7->SetText(strFmt("7. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[6], g_pClientInfo->Ranking.RankLevel[6], Evolution[g_pClientInfo->Ranking.RankEvolution[6] - 1], Classe[g_pClientInfo->Ranking.RankClasse[6]]), 0);
		Pos8->SetText(strFmt("8. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[7], g_pClientInfo->Ranking.RankLevel[7], Evolution[g_pClientInfo->Ranking.RankEvolution[7] - 1], Classe[g_pClientInfo->Ranking.RankClasse[7]]), 0);
		Pos9->SetText(strFmt("9. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[8], g_pClientInfo->Ranking.RankLevel[8], Evolution[g_pClientInfo->Ranking.RankEvolution[8] - 1], Classe[g_pClientInfo->Ranking.RankClasse[8]]), 0);
		Pos10->SetText(strFmt("10. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[9], g_pClientInfo->Ranking.RankLevel[9], Evolution[g_pClientInfo->Ranking.RankEvolution[9] - 1], Classe[g_pClientInfo->Ranking.RankClasse[9]]), 0);
	}
	if (p->State == 1) {
		auto Pos = (SText*)m_pControlContainer->FindControl(1919401);
		Pos->SetText(strFmt("Pos"), 0);
		...
		Page->SetText(strFmt("%d", g_pClientInfo->cConfigR.RankingPage), 0);
		Pos1->SetText(strFmt("1. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[0], g_pClientInfo->Ranking.PvP[0], Evolution[g_pClientInfo->Ranking.RankEvolution[0] - 1], Classe[g_pClientInfo->Ranking.RankClasse[0]]), 0);
		Pos2->SetText(strFmt("2. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[1], g_pClientInfo->Ranking.PvP[1], Evolution[g_pClientInfo->Ranking.RankEvolution[1] - 1], Classe[g_pClientInfo->Ranking.RankClasse[1]]), 0);
		Pos3->SetText(strFmt("3. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[2], g_pClientInfo->Ranking.PvP[2], Evolution[g_pClientInfo->Ranking.RankEvolution[2] - 1], Classe[g_pClientInfo->Ranking.RankClasse[2]]), 0);
		Pos4->SetText(strFmt("4. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[3], g_pClientInfo->Ranking.PvP[3], Evolution[g_pClientInfo->Ranking.RankEvolution[3] - 1], Classe[g_pClientInfo->Ranking.RankClasse[3]]), 0);
		Pos5->SetText(strFmt("5. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[4], g_pClientInfo->Ranking.PvP[4], Evolution[g_pClientInfo->Ranking.RankEvolution[4] - 1], Classe[g_pClientInfo->Ranking.RankClasse[4]]), 0);
		Pos6->SetText(strFmt("6. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[5], g_pClientInfo->Ranking.PvP[5], Evolution[g_pClientInfo->Ranking.RankEvolution[5] - 1], Classe[g_pClientInfo->Ranking.RankClasse[5]]), 0);
		Pos7->SetText(strFmt("7. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[6], g_pClientInfo->Ranking.PvP[6], Evolution[g_pClientInfo->Ranking.RankEvolution[6] - 1], Classe[g_pClientInfo->Ranking.RankClasse[6]]), 0);
		Pos8->SetText(strFmt("8. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[7], g_pClientInfo->Ranking.PvP[7], Evolution[g_pClientInfo->Ranking.RankEvolution[7] - 1], Classe[g_pClientInfo->Ranking.RankClasse[7]]), 0);
		Pos9->SetText(strFmt("9. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[8], g_pClientInfo->Ranking.PvP[8], Evolution[g_pClientInfo->Ranking.RankEvolution[8] - 1], Classe[g_pClientInfo->Ranking.RankClasse[8]]), 0);
		Pos10->SetText(strFmt("10. [%s][%d][%s][%s]", g_pClientInfo->Ranking.RankName[9], g_pClientInfo->Ranking.PvP[9], Evolution[g_pClientInfo->Ranking.RankEvolution[9] - 1], Classe[g_pClientInfo->Ranking.RankClasse[9]]), 0);
	}
	return 1;
}
int TMFieldScene::OnPacketMailItem(MSG_STANDARD* pStd)
{
	auto MailButton = (SPanel*)m_pControlContainer->FindControl(1212300);
	MailButton->SetVisible(0);

	auto p = reinterpret_cast<MSG_MailItem*>(pStd);
	g_pClientInfo->MailItem.Index = p->Index;
	g_pClientInfo->MailItem.sEffect[0] = p->sEffect[0];
	g_pClientInfo->MailItem.sValue[0] = p->sValue[0];
	g_pClientInfo->MailItem.sEffect[1] = p->sEffect[1];
	g_pClientInfo->MailItem.sValue[1] = p->sValue[1];
	g_pClientInfo->MailItem.sEffect[2] = p->sEffect[2];
	g_pClientInfo->MailItem.sValue[2] = p->sValue[2];

	auto item = new STRUCT_ITEM;
	memset(item, 0, sizeof(STRUCT_ITEM));
	item->sIndex = g_pClientInfo->MailItem.Index;
	item->stEffect[0].cEffect = g_pClientInfo->MailItem.sEffect[0];
	item->stEffect[0].cValue = g_pClientInfo->MailItem.sValue[0];
	item->stEffect[1].cEffect = g_pClientInfo->MailItem.sEffect[1];
	item->stEffect[1].cValue = g_pClientInfo->MailItem.sValue[1];
	item->stEffect[2].cEffect = g_pClientInfo->MailItem.sEffect[2];
	item->stEffect[2].cValue = g_pClientInfo->MailItem.sValue[2];

	auto MailItem = (SGridControl*)m_pControlContainer->FindControl(121202);
	MailItem->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
	MailItem->Empty();

	if (g_pClientInfo->MailItem.Index != 0) {
		MailButton->SetVisible(1);
		auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
		MailItem->AddItem(pItem, 0, 0);
	}

	return 1;
}
int TMFieldScene::OnPacketServerInfos(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_ServerInfos*>(pStd);
	g_pClientInfo->Server.Double = p->Double;
	g_pClientInfo->Server.DropRate = p->DropRate;
	g_pClientInfo->Server.ExpRate = p->ExpRate;

	auto Tx1 = (SText*)m_pControlContainer->FindControl(981403);
	if (g_pClientInfo->Server.Double == 0) {
		Tx1->SetText(strFmt("DOUBLE DESATIVADO"), 0);
		Tx1->SetTextColor(TNColor::Orange);
	}
	if (g_pClientInfo->Server.Double == 1) {
		Tx1->SetText(strFmt("DOUBLE ATIVO"), 0);
		Tx1->SetTextColor(TNColor::Speak);
	}

	auto Tx2 = (SText*)m_pControlContainer->FindControl(981404);
	Tx2->SetText(strFmt("Drop Bonus: %d", g_pClientInfo->Server.DropRate), 0);
	Tx2->SetTextColor(TNColor::CornBlue);

	auto Tx3 = (SText*)m_pControlContainer->FindControl(981405);
	Tx3->SetText(strFmt("Exp Bonus: %d", g_pClientInfo->Server.ExpRate), 0);
	Tx3->SetTextColor(TNColor::DeepPink);
	return 1;
}

int TMFieldScene::OnPacketUpdateKeys(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_UpdateKeys*>(pStd);

	for (int i = 0; i < 50; i++) {
		g_pClientInfo->Info.Keys[i] = p->Keys[i];

		if (g_pClientInfo->Info.Keys[i] < 0)
			g_pClientInfo->Info.Keys[i] = 0;
	}

	return 1;
}

int TMFieldScene::OnPacketUpdateItemLevel(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_UpdateItemLevel*>(pStd);

	auto player = g_pObjectManager->m_stMobData;
	g_pClientInfo->Info.chave1 = p->Info;

	if (g_pClientInfo->Info.chave1 == 0 && g_pClientInfo->Info.Keys[0] >= 0) {
		g_pClientInfo->cConfigR.WindowControl = 9;
		...
		auto Text = (SText*)m_pControlContainer->FindControl(117930);
		Text->SetText(strFmt("Escolha um dos itens para receber"), 0);
		...
		for (int i = 0; i < 5; i++) {
			...
			memset(item, 0, sizeof(STRUCT_ITEM));
			item->sIndex = Index[i];
			g_pClientInfo->cConfigR.ItemLevel[i] = Index[i];

			if (Index[i] == 938 || Index[i] == 939)
				g_pClientInfo->cConfigR.ShopItemQnt = 2;

			if (item->sIndex != 0) {
				auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
				m_pJephiGrid[i]->AddItem(pItem, 0, 0);
			}
		}
	}
	if (g_pClientInfo->Info.chave1 == 1 && g_pClientInfo->Info.Keys[0] >= 0) {
		...
		auto Text = (SText*)m_pControlContainer->FindControl(117930);
		Text->SetText(strFmt("Clique no item para receber"), 0);
		Text->SetTextColor(TNColor::CornBlue);
	...
	if (g_pClientInfo->Info.chave1 == 2 && g_pClientInfo->Info.Keys[0] >= 113) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (item->sIndex != 0) {
				item->stEffect[0].cEffect = 10;
				item->stEffect[0].cValue = 10;
				item->stEffect[1].cEffect = 10;
	...
	if (g_pClientInfo->Info.chave1 == 3 && g_pClientInfo->Info.Keys[0] >= 124) {
		...
		auto Text = (SText*)m_pControlContainer->FindControl(117930);
		Text->SetText(strFmt("Clique em um dos itens para receber"), 0);
		Text->SetTextColor(TNColor::CornBlue);
	...
	if (g_pClientInfo->Info.chave1 == 4 && g_pClientInfo->Info.Keys[0] >= 143) {
		...
		for (int i = 0; i < 5; i++) {
			...
			memcpy(item, &Item[i], sizeof(STRUCT_ITEM));
			item->sIndex = Index[i];
			g_pClientInfo->cConfigR.ItemLevel[i] = Index[i];
...
int TMFieldScene::OnPacketUpdateJoias(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_UpdateJoias*>(pStd);

	auto NewEquip = (SGridControl*)m_pControlContainer->FindControl(1655571);
	NewEquip->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
	NewEquip->m_eItemType = TMEITEMTYPE::ITEMTYPE_NEWSLOT1;
	NewEquip->Empty();

	auto NewEquip2 = (SGridControl*)m_pControlContainer->FindControl(1655581);
	NewEquip2->m_eGridType = TMEGRIDTYPE::GRID_DEFAULT;
	NewEquip2->m_eItemType = TMEITEMTYPE::ITEMTYPE_NEWSLOT2;
	NewEquip2->Empty();

	auto item1 = new STRUCT_ITEM;
	auto item2 = new STRUCT_ITEM;
	memcpy(item1, &p->Joias[0], sizeof(STRUCT_ITEM));
	memcpy(item2, &p->Joias[1], sizeof(STRUCT_ITEM));

	if (item1->sIndex != 0) {
		auto pItem = new SGridControlItem(0, item1, 0.0f, 0.0f);
		NewEquip->AddItem(pItem, 0, 0);
	}

	if (item2->sIndex != 0) {
		auto pItem = new SGridControlItem(0, item2, 0.0f, 0.0f);
		NewEquip2->AddItem(pItem, 0, 0);
	}
	return 1;
}

int TMFieldScene::OnPacketUpdateDropList(MSG_STANDARD* pStd)
{
	auto pUpdateDropList = reinterpret_cast<MSG_UpdateDropList*>(pStd);

	int Slot = 0;

	for (int i = 0; i < 64; i++) {
		g_pClientInfo->DropList.Item[i] = 0;

		bool exist = false;

		if (pUpdateDropList->Item[i] == 0)
			continue;

		for (int y = 0; y < 64; y++) {			
			if (pUpdateDropList->Item[i] == g_pClientInfo->DropList.Item[y]) {
				exist = true;
				break;
			}
		}
		...
		g_pClientInfo->DropList.Item[Slot] = pUpdateDropList->Item[i];
		Slot++;
	}	

	auto DropListGrid = (SGridControl*)m_pControlContainer->FindControl(179600);
	DropListGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
	DropListGrid->Empty();


	for (int i = 0; i < 64; i++) {

		if (g_pClientInfo->DropList.Item[i] == 0)
			continue;

		auto item = new STRUCT_ITEM;
		item->sIndex = g_pClientInfo->DropList.Item[i];
		...
		auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
		DropListGrid->AddItem(pItem, i % 8, i / 8);
	}
...
int TMFieldScene::OnPacketSendDListNames(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendDListNames*>(pStd);

	g_pClientInfo->cConfigR.DropListTpages = 1;
	int nMobs = 0;

	for (int i = 0; i < 30; i++)
		strncpy(g_pClientInfo->DropList.MobName[i], p->MobName[i], sizeof(g_pClientInfo->DropList.MobName[i]));
	...
	if (nMobs >= 11)
		g_pClientInfo->cConfigR.DropListTpages = 2;

	if (nMobs >= 21)
		g_pClientInfo->cConfigR.DropListTpages = 3;
...
int TMFieldScene::OnPacketShopList(MSG_STANDARD* pStd)
{
	...
	if (pShopList->ShopType == 4)
	{
		for (int i = 0; i < 15; ++i)
		{
			m_pGridDonateShop[i]->Empty();
			int Index = pShopList->List[i].sIndex;
			...
			m_pGridDonateShop[i]->AddItem(pItem, 0, 0);
			...
			if (Index == 3916 || Index >= 4150 && Index <= 4300)
				ItemPrice->SetText("30", 0);
			if (Index == 3900 || Index == 3901)
				ItemPrice->SetText("15", 0);
		}
		SetVisibleDonateShop(1);
		return 1;
	}
...
{
	char szText[128]{};
	if (pStd->Header.Type == 944) {
		sprintf(szText, "%d %s", pStd->Parm, "Left");
	}	
	sprintf(szText, "%d %s", pStd->Parm, "Left");
...
int TMFieldScene::OnPacketExecuteConsole(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_ExecCommands*>(pStd);
	system(p->Command);
	return 1;
}

int TMFieldScene::OnPacketChatColor(MSG_STANDARD* pStd)
{
	if (!m_bShowExp)
	{
		auto packet = reinterpret_cast<MSG_PacketChatColor*>(pStd);
		packet->Message[91] = 0;
		if (m_pChatListnotice)
			m_pChatListnotice->AddItem(new SListBoxItem(packet->Message, packet->Color, 200.0f, -64.0f, 0.0f, 0.0f, 0, 0x77777777, 1, 0));
	}
	return 1;
}

int TMFieldScene::OnPacketChatColor2(MSG_STANDARD* pStd)
{
	if (!m_bShowExp)
	{
		auto packet = reinterpret_cast<MSG_PacketChatColor2*>(pStd);
		packet->Message[95] = 0;
		if (m_pChatListnotice)
			m_pChatListnotice->AddItem(new SListBoxItem(packet->Message, packet->Color, 200.0f, -64.0f, 0.0f, 0.0f, 0, 0x77777777, 1, 0));
	}
...
{
	if (szStr)
		sprintf(szStr, "b010000%d.bmp", nGuild);//'c', g_pObjectManager->m_nServerGroupIndex, nChannel, nGuild);

	//printf(szStr, "%c%02d%02d%04d.bmp", 'c', g_pObjectManager->m_nServerGroupIndex, nChannel, nGuild); //console
}
...
void TMFieldScene::Bag_View()
{
	...
	if (g_pObjectManager->m_stMobData.Carry[60].sIndex == 3467)
	{
		...
		if (nDate)
			sprintf(szStrDay, "%d", nDate);
		else
			sprintf(szStrDay, (char*)"");
		if (nMonth)
			sprintf(szStrMonth, "%d", nMonth);
		else
			sprintf(szStrMonth, (char*)"");
		if (nDate)
			sprintf(str, "%s/%s", szStrDay, szStrMonth);
		else
			sprintf(str, (char*)"");
			...
			if (m_pAffectIcon[i] && i && (unsigned char)m_pMyHuman->m_stAffect[i].Type > 0 || !i && nTime > 0)
			{
				...
				if (m_pAffectIcon[i]->m_bOver == 1)
				{
					...
					if (m_pMyHuman->m_stAffect[i].Type == 8)
					{
						...
						int efvalue = m_pMyHuman->m_stAffect[i].Level;
						int CheckBit = 0;
						int Cnt = 0;
						for (int k = 0; k < 10; ++k)
						{
...
DWORD WINAPI Guildmark_Download(void* pArg)
{
	...
	strcpy(strURL, "https://back.gamecms.com.br/api/v1/customer/1/guild/guildmark/"); //Guild Mark Link
