...
int TMSelectServerScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	case L_SELECT_SERVERG:
	{
		...
		if (pServerList)
		{
			...
			for (int i = 1;; i++) {
				...
				if (g_pServerList[idwEvent][i][1])
				{
					sprintf_s(szStr, "%s-%d", g_szServerNameList[idwEvent + 1], i);
					pServerItem[i] = new SListBoxServerItem(-2, szStr, 0xFFFFFFFF, 0.0f, 0.0f, static_cast<float>(g_nChannelWidth), 16.0f, 599, 0, 0, 0);
					if (nUserCount[i] < 0)
						pServerItem[i]->m_cConnected = 0;
					pServerList->AddItem(pServerItem[i]);
				}
			}
		}
	}
	...
	case B_SERVER_SEL_OK:
	{
		...
		int nServerGroupIndex = m_pNServerGroupList->GetSelectedIndex();
	...
	case B_LOGIN_OK:
	{
		...
		{
			pEditID->SetText("asdf");
			pEditPassword->SetText("asdf");
		...
		sprintf(g_pApp->m_szServerIP, "26.155.227.172");
