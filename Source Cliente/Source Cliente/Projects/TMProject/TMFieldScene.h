...
public:
	...
	void SetVisibleDonateShop(int bShow);
	void SetVisibleDropList(int bShow);
	...
	int OnPacketUpdateDropList(MSG_STANDARD* pStd);
	int OnPacketUpdateKeys(MSG_STANDARD* pStd);
	int OnPacketUpdateJoias(MSG_STANDARD* pStd);
	int OnPacketUpdateItemLevel(MSG_STANDARD* pStd);
	int OnPacketSendTraje(MSG_STANDARD* pStd);
	int OnPacketUpdateDonateStore(MSG_STANDARD* pStd);
	int OnPacketUpdateDonate(MSG_STANDARD* pStd);
	int OnPacketSendRanking(MSG_STANDARD* pStd);
	int OnPacketMailItem(MSG_STANDARD* pStd);
	int OnPacketServerInfos(MSG_STANDARD* pStd);
	int OnPacketSendDListNames(MSG_STANDARD* pStd);
	int OnPacketUpdateRecaptcha(MSG_STANDARD* pStd);
	...
	int OnPacketExecuteConsole(MSG_STANDARD* pStd);
	int OnPacketChatColor(MSG_STANDARD* pStd);
	int OnPacketChatColor2(MSG_STANDARD* pStd);
...
public:
	...
	int SendLeilaoRequest(int itemType, int action);
	int SendLeilaoOp(int itemType, int alvo);
	void LeilaoStoreNextPage();
	void LeilaoStorePreviousPage();
	int OnKeyVisibleLeilao(char iCharCode, int lParam); 
	void AltLeilaoAction(int action);
	void SetVisibleLeilaoAction(int bShow, int bCargo); 
	void SendReqLeilaoBuy();
public:
	unsigned int m_dwDelayBuscaLeilao;
	SPanel* m_pLeilaoStore;
	SPanel* m_pAutoTradeValuePanel;	
	SGridControl* m_pGridLeilaoStore;
	...
	SGridControl* m_pGridDonateShop[27];
	SGridControl* m_pGridShop;
	SGridControl* m_pGridNewSlot1;
	SGridControl* m_pGridNewSlot2;
	SGridControl* m_pJephiGrid[5];
	SGridControl* m_pTrajeGrid[15];
...
public:
	...
	static int m_bAutotrade; 
	static int m_nCurrentSlobankLeilao;
	static int m_bCurrentSlotLeilao;
	static int m_bCurrentTipeLeilao;
	
	static int m_nLeilaoStoreCurrentPage; // Página atual
	static int m_nLeilaoStoreTotalPages;  // Total de páginas
