...
void SControl::SetRbutton(int x)
{
}
...
int SCursor::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	{
		if (pFScene->m_pDescPanel->m_bVisible == 0)
		{
			if (dwFlags == WM_RBUTTONDOWN)
			{
				SetCamfree(1);
			}
			else if (dwFlags == WM_RBUTTONUP)
			{
				SetCamfree(0);
			}
		}
		else if (pFScene->m_pDescPanel->m_bVisible == 1)
		{
			SetCamfree(0);
		}
	}
	...
	if (dwFlags == WM_MOUSEMOVE)
	{ 
		if (!g_pEventTranslator->m_bRCamfree) {
			SetPosition(nX, nY);
		}
	}  
	return SControl::OnMouseEvent(dwFlags, wParam, nX, nY);
...
void SCursor::SetCamfree(int indiceStatus)
{
	g_pEventTranslator->m_bRCamfree = indiceStatus;
}
...
int SEditableText::OnCharEvent(char iCharCode, int lParam)
{
	...
	if (iCharCode == VK_BACK)
	{
		...
		if (g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_FIELD)
		{
			...
			if (pFScene->m_pLeilaoStore && pFScene->m_pLeilaoStore->m_bVisible && m_bFocused && m_dwControlID == 3002213)
			{
				if (pFScene->m_bCurrentSlotLeilao >= 0 && pFScene->m_dwDelayBuscaLeilao + 1000 <= g_pApp->m_pTimerManager->GetServerTime())
				{
					pFScene->SendLeilaoRequest(pFScene->m_stAutoTrade.List[pFScene->m_bCurrentSlotLeilao].Categoria, 0);
					pFScene->m_dwDelayBuscaLeilao = g_pApp->m_pTimerManager->GetServerTime();
				}
			}
		}
		int nTextLen = strlen(m_strText);
...
void SEditableText::FrameMove2(stGeomList* pDrawList, TMVector2 ivParentPos, int inParentLayer, int nFlag)
{
	...
	if (g_pCurrentScene->GetSceneType() == ESCENE_TYPE::ESCENE_FIELD)
	{
		...
		auto BuscaleilaoStore = static_cast<SEditableText*>(pFScene->m_pControlContainer->FindControl(3002213));

		auto pGridLeilaoStore = (SGridControl*)pFScene->m_pControlContainer->FindControl(3200011);

		if (BuscaleilaoStore == nullptr)
			return;

		if (pGridLeilaoStore == nullptr)
			return;
		...
		if (pFScene->m_pLeilaoStore && pFScene->m_pLeilaoStore->m_bVisible && m_bFocused && m_dwControlID == 3002213)
		{
			...
			std::cout << "\n m_GCText.strString " << m_GCText.strString;

			std::string searchString = m_GCText.strString;
			std::transform(searchString.begin(), searchString.end(), searchString.begin(), ::tolower); // Converter para minúsculas
			// Verifica se a string de busca está vazia e o grid está vazio
			if (searchString.empty())
			{
				static int DelayCount = 0;

				if (pFScene->m_dwDelayBuscaLeilao + 5000 <= g_pApp->m_pTimerManager->GetServerTime() && pFScene->m_bCurrentSlotLeilao >= 0)
				{
					if (pFScene->m_pInvenPanel->m_bVisible)
					{
						pFScene->SendLeilaoRequest(pFScene->m_bCurrentTipeLeilao, 0);
					}
					else
					{
						pFScene->SendLeilaoRequest(pFScene->m_bCurrentTipeLeilao, 1);
					}
					...
					if (DelayCount >= 4)
					{
						pFScene->m_pControlContainer->SetFocusedControl(0);
			...
			else // Caso a string de busca não esteja vazia
			{
				for (int i = 0; i < pGridLeilaoStore->m_nNumItem; )
				{
					auto pGridCurrent = pGridLeilaoStore->m_pItemList[i];
					if (!pGridCurrent)
					{
						++i;
						continue;
					...
					auto IndiceItem = pGridCurrent->m_pItem->sIndex;
					if (IndiceItem <= 0)
					{
						++i;
						continue;
					...
					std::string itemName = g_pItemList[IndiceItem].Name;
					std::transform(itemName.begin(), itemName.end(), itemName.begin(), ::tolower); // Converter para minúsculas
					 
					if (itemName.find(searchString) == std::string::npos)
					{
						pGridLeilaoStore->RemoveItemAt(pGridCurrent->m_nCellIndexX, pGridCurrent->m_nCellIndexY);
					}
					else
					{ 
						++i;  
		...
		0)
{
	...
	m_pBusyProgress = new SProgressBar(nTextureSet, nCount, 600, inWidth - 58.0f, 6.0f, 40.0f, 6.0f, dwCol1, dwCol2, 1);
}
