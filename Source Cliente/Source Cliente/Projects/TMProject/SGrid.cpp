...
int SGridControl::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	else if (dwFlags == 513 && g_pEventTranslator->m_bShift)
	{
		...
		pFScene->m_nCurrentSlobankLeilao = -1;
...
void SGridControl::FrameMove2(stGeomList* pDrawList, TMVector2 ivParentPos, int inParentLayer, int nFlag)
{
	...
	if (m_eItemType != TMEITEMTYPE::ITEMTYPE_NONE
		...
		&& m_eGridType == TMEGRIDTYPE::GRID_LEILAOSTORE)
	{
		if (!m_pItemList[0])
			return;
...
int SGridControl::CanChangeItem(SGridControlItem* ipNewItem, int inCellIndexX, int inCellIndexY, int bOnlyCheck)
{
	...
	if (!sType)
	{
		...
		if (ipNewItem->m_pItem->sIndex >= 4468 && ipNewItem->m_pItem->sIndex <= 4482)
			nItemType = 131072;

		if (ipNewItem->m_pItem->sIndex >= 4483 && ipNewItem->m_pItem->sIndex <= 4497)
			nItemType = 65536;
		...
		if (m_dwControlID == 1655581)
		{
			if (nItemType == 65536) {			
				return 1;
			}
		}
		if (m_dwControlID == 1655571)
		{
			if (nItemType == 131072) {
				return 1;
			}
...
void SGridControl::BuyItem(int nCellX, int nCellY, int ItemIndex)
{
	int Field = SGridControl::GetControlID();

	//Nova Loja Donate
	if (m_eGridType == TMEGRIDTYPE::GRID_SHOP && (Field >= 999301 && Field <= 999315))
	{
		auto pScene = static_cast<TMFieldScene*>(g_pCurrentScene);
	...
	else if (m_eGridType == TMEGRIDTYPE::GRID_SHOP)
	{
		...
		if (pItem)
		{
			...
			stFiltro.Header.ID = g_pCurrentScene->m_pMyHuman->m_dwID;
			stFiltro.Header.Type = _MSG_SendFiltro;
			stFiltro.ItemId = ItemIndex;
			SendOneMessage((char*)&stFiltro, sizeof(stFiltro));
...
int SGridControl::TradeItem(int nCellX, int nCellY)
{
	...
	{
		if (pFScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD) /*Clicou no guarda carga para enviar abrir o menu de adicionar valor no item e escolher qual tipo de valor o item vai ter*/
		{  
			...
			auto pInputGold = (SPanel*)pFScene->m_pControlContainer->FindControl(565885);
			auto pItem = pFScene->m_pCargoGridList[page]->GetItem(nCellX, nCellY);
			auto gGridSlotValuePane = (SGridControl*)pFScene->m_pControlContainer->FindControl(565225);

			auto pRunAutoTrade = (SButton*)pFScene->m_pControlContainer->FindControl(3000011);

			if (!pRunAutoTrade)
				return 1;

			if (!gGridSlotValuePane)
				return 1;
			...
			{
				auto Generico1 = static_cast<SButton*>(pFScene->m_pControlContainer->FindControl(565811));
				auto Generico2 = static_cast<SButton*>(pFScene->m_pControlContainer->FindControl(565812));
				auto Generico3 = static_cast<SButton*>(pFScene->m_pControlContainer->FindControl(565813));

				auto pText = (SText*)pFScene->m_pControlContainer->FindControl(565888);
				auto pNomeItem = (SText*)pFScene->m_pControlContainer->FindControl(565819);
				...
				pFScene->m_nCurrentSlobankLeilao = 40 * page + 5 * pItem->m_nCellIndexY + pItem->m_nCellIndexX;
				...
				if (gGridSlotValuePane)
				{
					gGridSlotValuePane->Empty();

					char str[128]{};
					snprintf(str, sizeof str, "%s", g_pItemList[pItem->m_pItem->sIndex].Name);
					pNomeItem->SetText(str, 0);
					 
					auto pNewItem = new STRUCT_ITEM;

					memcpy(pNewItem, pItem->m_pItem, sizeof(STRUCT_ITEM));

					pFScene->m_bCurrentTipeLeilao = -1; 
					auto pNewControlItem = new SGridControlItem(0, pNewItem, 0.0f, 0.0f); 
					gGridSlotValuePane->AddItem(pNewControlItem, 0, 0);//adiciona no painel de tipo de valores o item que o usuario clicou.
					gGridSlotValuePane->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
				}
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_LEILAOSTORE)//AQUI ACESSA A LOJA
	{ 
		auto pItem = GetItem(nCellX, nCellY);
		if (!pItem)
			return 1;

		auto pGridLeilaoStore = (SGridControl*)pFScene->m_pControlContainer->FindControl(3200011);

		if (!pGridLeilaoStore)
			return 1;

		auto pGrid = pGridLeilaoStore; 

		if (pItem && pFScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
		{
			auto pInputGold = (SPanel*)pFScene->m_pControlContainer->FindControl(565885);

			if (pInputGold && pInputGold->m_bVisible)
			{
				return 1;
			}
			if (pFScene->m_pDescPanel && pFScene->m_pDescPanel->m_bVisible)
			{
			...
			if (SlotClicado < MAX_SLOT_LEILAO_AUTOTRADE)
			{
				...
				auto bb2 = pFScene->m_pGridLeilaoStore->GetItem(nCellX, nCellY);

				if (List.Item.sIndex && List.Categoria != 0)
				{
					if (bb2->m_GCObj.dwColor != 0xFFFF0000)
					{
						pFScene->m_pMessageBox->m_dwMessage = m_dwControlID;
						pFScene->m_bCurrentSlotLeilao = static_cast<int>(SlotClicado);
						pFScene->m_bCurrentTipeLeilao = pFScene->m_stAutoTrade.List[pFScene->m_bCurrentSlotLeilao].Categoria;

						char szMessage[128]{};
						sprintf(szMessage, "Deseja acessar loja %s ? ", pFScene->m_stAutoTrade.List[pFScene->m_bCurrentSlotLeilao].LojinhaName);
						pFScene->m_pMessageBox->SetMessage(szMessage, 3200011, 0);
						pFScene->m_pMessageBox->SetVisible(1);

						pGridLeilaoStore->m_eGridType = TMEGRIDTYPE::GRID_LEILAOSTORE_BUY;
	...
	if (m_eGridType == TMEGRIDTYPE::GRID_LEILAOSTORE_BUY)//AQUI COMPRA O ITEM
	{
		auto pItem = GetItem(nCellX, nCellY);
		if (!pItem)
			return 1;
		...
		if (pItem && pFScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
		{
			...
			if (SlotClicado < MAX_SLOT_LEILAO_AUTOTRADE)
			{
				...
				if (List.Item.sIndex && List.Categoria != 0)
				{
					...
					{
						char szMessage[128]{};
						sprintf(szMessage, "Confirmar compra em %s", pFScene->m_stAutoTrade.List[pFScene->m_bCurrentSlotLeilao].LojinhaName);
						pFScene->m_pMessageBox->SetMessage(szMessage, 3200012, 0);
						pFScene->m_pMessageBox->SetVisible(1);

						pFScene->m_pMessageBox->m_dwMessage = 3200012;
						pFScene->m_bCurrentSlotLeilao = static_cast<int>(SlotClicado);
						pFScene->m_bCurrentTipeLeilao = pFScene->m_stAutoTrade.List[pFScene->m_bCurrentSlotLeilao].Categoria;
...
void SGridControl::SwapItem(int nCellX, int nCellY, int nCellVWidth, int nCellVHeight, STRUCT_ITEM* pItem)
{
		...
		(page != 3 || g_pObjectManager->m_stMobData.Carry[61].sIndex == 3467))
	{
		...
		else if (m_eItemType == TMEITEMTYPE::ITEMTYPE_NEWSLOT1 || m_eItemType == TMEITEMTYPE::ITEMTYPE_NEWSLOT2) {
			short sSrcType = CheckType(m_eItemType, m_eGridType);
			short sSrcPos = CheckPos(m_eItemType);
...
int SGridControl::MouseOver(int nCellX, int nCellY, int bPtInRect)
{
	...
	else if (pDescNameText)
	{
		...
		else
		{
			...
			if (nRefLevel >= 65)
				sprintf(szText, "%s [%c]", g_pItemList[pItem->m_pItem->sIndex].Name, nRefLevel);
			else if (pItem->m_pItem->sIndex >= 2360 && pItem->m_pItem->sIndex <= 2389 && (pItem->m_pItem->stEffect[2].cValue >= 10 && pItem->m_pItem->stEffect[2].cValue < 22))
			...
			else if (pItem->m_pItem->sIndex >= 2360 && pItem->m_pItem->sIndex <= 2389 && (pItem->m_pItem->stEffect[2].cValue >= 22))
				sprintf(szText, g_pMessageStringTable[483], g_pItemList[pItem->m_pItem->sIndex].Name, g_pItemList[pItem->m_pItem->stEffect[2].cValue + 4478].Name); //Escreve o nome do traje de montaria
			else
				sprintf(szText, "%s", g_pItemList[pItem->m_pItem->sIndex].Name);
	...
	//Item Description
	int sIndex = pItem->m_pItem->sIndex;
	char Porcentagem[2] = "%";
	...
	if (sIndex == 5785) {
		...
		pFScene->m_pParamText[5]->SetText(strFmt("[APENAS UM ITEM RECEBIDO POR CAIXA]"), 0);
	}
	if (sIndex == 5783) {
		...
		pFScene->m_pParamText[5]->SetText(strFmt("Ao fechar a janela, interrompe o uso do alicate"), 0);
	}
	if (sIndex == 5784) {
		pFScene->m_pParamText[3]->SetTextColor(Laranja);
	...
	if (sIndex == 788 || sIndex == 789 || sIndex == 792 || sIndex == 793 || sIndex == 794) {
		...
		if (pItem->m_pItem->stEffect[0].cEffect == 43)
			Ref = pItem->m_pItem->stEffect[0].cValue;
	...
	if (sIndex == 5614) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITEM PREMIUM]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(RoxoRosa);
		pFScene->m_pParamText[2]->SetText(strFmt("Altere sua classe usando esse item"), 0);
	...
	if (sIndex == 3331) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[PASSE DO EVENTO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Ouro);
		pFScene->m_pParamText[2]->SetText(strFmt("Habilita Double Hit"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
	...
	if (sIndex == 5720) {
		...
		pFScene->m_pParamText[6]->SetText(strFmt("Sleipnir, Svaldfire"), 0);
	...
	if (sIndex == 5722) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Poeira e Resto de Oriharucom"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Poeira e Resto de Lactolerium"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5723) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Classe D"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Classe E"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5724) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Vela do Coveiro, Colheira do Jardineiro"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Cura do Batedor, Mana do Batedor"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Emblema do Guarda, Pista de Runas"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5725) {
	...
	if (sIndex == 5730) {
		...
		pFScene->m_pParamText[5]->SetTextColor(Azul);
		pFScene->m_pParamText[5]->SetText(strFmt("Bota Rake"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5731) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Arco Divino"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Garra Draconiana"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Espada Vorpal"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5732) {
		...
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Vingadora"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5733) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Skytalos"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Khyrius"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Caliburn"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5734) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Elmo Embutido (N/M/A)"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		...
		pFScene->m_pParamText[6]->SetTextColor(Azul);
		pFScene->m_pParamText[6]->SetText(strFmt("Bota Embutida (N/M/A)"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5735) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Elmo Mortal"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		...
		pFScene->m_pParamText[6]->SetTextColor(Azul);
		pFScene->m_pParamText[6]->SetText(strFmt("Bota Mortal"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5736) {
		...
		pFScene->m_pParamText[5]->SetTextColor(Azul);
		pFScene->m_pParamText[5]->SetText(strFmt("Bota Flamejante"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5737) {
	...
	if (sIndex == 5739) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Balmung"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Gleipnir"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Thrasytes"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5740) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Elmo Elemental (N/M/A)"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		...
		pFScene->m_pParamText[6]->SetTextColor(Azul);
		pFScene->m_pParamText[6]->SetText(strFmt("Bota Elemental (N/M/A)"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5741) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Elmo do Corvo"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		...
		pFScene->m_pParamText[6]->SetTextColor(Azul);
		pFScene->m_pParamText[6]->SetText(strFmt("Bota do Corvo"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5742) {
	...
	if (sIndex == 5744) {
		...
		pFScene->m_pParamText[2]->SetText(strFmt("Asa Draconiana"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Foice Platinada"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Karikas"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5745) {
		...
		pFScene->m_pParamText[3]->SetText(strFmt("Gleipnir"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Hermai"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5746) {
		...
		pFScene->m_pParamText[1]->SetText(strFmt("[ITENS INCLUSOS NO FILTRO]"), 0);
		pFScene->m_pParamText[2]->SetTextColor(Azul);
		pFScene->m_pParamText[2]->SetText(strFmt("Chapeu de Mytril (N/M/A)"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		...
		pFScene->m_pParamText[6]->SetTextColor(Azul);
		pFScene->m_pParamText[6]->SetText(strFmt("Bota de Mytril (N/M/A)"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5747) {
	...
	if (sIndex == 5751) {
		...
		pFScene->m_pParamText[2]->SetText(strFmt("Skytalos"), 0);
		pFScene->m_pParamText[3]->SetTextColor(Azul);
		pFScene->m_pParamText[3]->SetText(strFmt("Caliburn"), 0);
		pFScene->m_pParamText[4]->SetTextColor(Azul);
		pFScene->m_pParamText[4]->SetText(strFmt("Eirenus"), 0);
		pFScene->m_pParamText[5]->SetTextColor(Azul);
		pFScene->m_pParamText[5]->SetText(strFmt("Neorion"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	...
	else
	{
		...
		else if (pItem->m_pItem->sIndex >= 4150 && pItem->m_pItem->sIndex <= 4250
	...
	if (m_eGridType != TMEGRIDTYPE::GRID_SHOP)
	{
		...
		{ 
			...
			{
				auto& List = pFScene->m_stAutoTrade.List[selectedItemIndex];
				 
				if (List.Item.sIndex  && List.Categoria != 0)
				{
					int nPrice = List.Price;

					int nTipy = List.Categoria;

					char szPrice[128];
					char szPrice2[128]; 

					std::string formattedPrice = FormatPrice(nPrice);

					if (nTipy == 1) {
						sprintf(szPrice, "%s %s", formattedPrice.c_str(), "Donate");
					}
					else if (nTipy == 2) {
						sprintf(szPrice, "%s %s", formattedPrice.c_str(), "Gold");
					}
					else if (nTipy == 3) {
						sprintf(szPrice, "%s %s", formattedPrice.c_str(), "Bis");
					}


					char szText[128] = ""; // Inicializa szText com uma string vazia

					 
					sprintf(szText, "%s", szPrice);
					pParamText->SetText(szText, 0);
				}
	...
	else
	{
		...
		else
		{
			...
			for (int a = 0; a <= 2; a++)
			{
				if (pItem->m_pItem->stEffect[a].cEffect == 92)
				{
					sprintf(szText, "Honra: %d", pItem->m_pItem->stEffect[a].cValue);
					modded = true;
					break;
				}
			}
			...
			if (!modded)
			{
				...
				{
					sprintf(szText, "Entregue 1 Emblema Orc");
				}

				else if (pItem->m_pItem->sIndex >= 501 && pItem->m_pItem->sIndex <= 506)
				{
					sprintf(szText, "Entregue 1 Emblema Orc");
				}

				else if (pItem->m_pItem->sIndex >= 651 && pItem->m_pItem->sIndex <= 653)
				{
					sprintf(szText, "Entregue 1 Emblema Orc");
				}
...
/*Metodos Busca sistema leilao store*/
std::string SGridControl::FormatPrice(int price) {
...
int SGridControl::GetIndexFromCoordinates(int x, int y)
{
	return y * m_nColumnGridCount + x;
}
void SGridControl::RemoveItemAt(int x, int y)
{
	int index = GetIndexFromCoordinates(x, y);
	if (index >= 0 && index < m_nNumItem)
	{
		SAFE_DELETE(m_pItemList[index]);
		m_pItemList[index] = nullptr;
		CompactGrid();
	}
}
void SGridControl::CompactGrid()
{
	int nextEmptySlot = 0;
	for (int i = 0; i < m_nNumItem; ++i)
	{
		if (m_pItemList[i] != nullptr)
		{
			if (i != nextEmptySlot)
			{
				m_pItemList[nextEmptySlot] = m_pItemList[i];
				m_pItemList[nextEmptySlot]->m_nCellIndexX = nextEmptySlot % m_nColumnGridCount;
				m_pItemList[nextEmptySlot]->m_nCellIndexY = nextEmptySlot / m_nColumnGridCount;
				m_pItemList[i] = nullptr;
			}
			++nextEmptySlot;
		}
	}
	m_nNumItem = nextEmptySlot;
}
void SGridControl::SwapItems(int index1, int index2)
{
	if (index1 >= 0 && index1 < m_nNumItem && index2 >= 0 && index2 < m_nNumItem)
	{
		...
		m_pItemList[index2]->m_nCellIndexX = index2 % m_nColumnGridCount;
		m_pItemList[index2]->m_nCellIndexY = index2 / m_nColumnGridCount;
	}
}
void SGridControl::SortItems(std::function<bool(SGridControlItem*, SGridControlItem*)> compare)
{
	...
	for (int i = 0; i < m_nNumItem; ++i)
	{
		...
		m_pItemList[i]->m_nCellIndexY = i / m_nColumnGridCount;
	}
}
